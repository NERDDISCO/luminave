...
return idx === -1 ? null : this.frames[idx]
  ...
  value(time, interpolator, out) {
    ...
    return this.lerper.lerp(startFrame.value, endFrame.value, t, out)
  }

  interpolation(time) {
    if (this.frames.length === 0) {
      return this.vec3.set(temp, -1, -1, 0)
    }
    ...
    if (prev === -1 || prev === this.frames.length - 1) {
      ...
      return this.vec3.set(temp, prev, prev, 0)
      ...
      const t = this.lerper.range(startFrame.time, endFrame.time, time)

      // provide interpolation factor
      return this.vec3.set(temp, prev, prev + 1, t)
