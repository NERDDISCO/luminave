...
</custom-style>`;template$7.setAttribute("style","display: none;");document.head.appendChild(template$7.content);const template$8=html`
...
`;var buttons$1={buttons:buttons};class ConnectButton extends connect(store)(LitElement){static get properties(){return{label:{type:String},type:{type:String},connected:{type:Boolean},connections:{type:Object}}}_stateChanged(state){this.connections=getConnections(state);this.connected=this.connections[this.type].connected}computeClass(connected){return connected?"primary":"warning"}render(){const{connected,label}=this;return html$1`
      ${buttons}

      <paper-button class="${this.computeClass(connected)}">${label}</paper-button>
    `}}customElements.define("connect-button",ConnectButton);class UsbDmxManager extends connect(store)(LitElement){constructor(){super();this.controller=new Controller({filters:[{vendorId:9025,productId:32822},{vendorId:9025,productId:32823},{vendorId:10755,productId:32832}]});this.controller.autoConnect().then(()=>{this.connect()}).catch(error=>{console.error(error);store.dispatch(connectUsb(!1))})}static get properties(){return{usbConnected:{type:Boolean},universes:{type:Array}}}_stateChanged(state){this.usbConnected=getUsbDmxControllerConnected(state);this.universes=getUniverses(state)}connectedCallback(){super.connectedCallback();window.addEventListener("send-universe-to-usb-dmx-controller",this.listenSendUniverse.bind(this))}disconnectedCallback(){super.disconnectedCallback();window.removeEventListener("send-universe-to-usb-dmx-controller",this.listenSendUniverse.bind(this))}listenSendUniverse(){if(this.usbConnected){this.controller.send(this.universes[0].channels).catch(error=>{console.error(error.code,error);switch(error.code){case 8:case 19:case 20:store.dispatch(connectUsb(!1));break;default:break;}})}}handleConnectButtonClick(){if(this.usbConnected){if(this.controller.device===void 0){store.dispatch(connectUsb(!1))}else{this.controller.disconnect().then(()=>{store.dispatch(connectUsb(!1))},error=>{console.error(error)})}}else{this.enable()}}enable(){this.controller.enable().then(()=>{this.connect()}).catch(error=>{console.error(error)})}connect(){this.controller.connect().then(()=>{store.dispatch(connectUsb(!0))},error=>{store.dispatch(connectUsb(!1));console.error(error)})}render(){return html$1`
      <connect-button type="usb" label="USB" @click="${()=>this.handleConnectButtonClick()}"></connect-button>
    `}}customElements.define("usb-dmx-manager",UsbDmxManager);class ModvManager extends connect(store)(LitElement){static get properties(){return{live:{type:Boolean},connected:{type:Boolean},url:{type:String}}}_stateChanged(state){this.live=getLive(state);this.connected=getModvConnected(state)}connectedCallback(){super.connectedCallback();this.url="ws://localhost:3000/luminave";if(this.connected){this.createWebsocket()}}handleClick(){if(this.connected){this.socket.close();store.dispatch(connectModv(!1))}else{this.createWebsocket()}}createWebsocket(){this.socket=new WebSocket(this.url);this.socket.addEventListener("open",()=>{console.info("modV WebSocket opened to",this.url);store.dispatch(connectModv(!0))});this.socket.addEventListener("close",event=>{console.info("modV WebSocket closed:",event);store.dispatch(connectModv(!1))});this.socket.addEventListener("error",error=>{console.error("modV WebSocket error:",error);store.dispatch(connectModv(!1))});this.socket.addEventListener("message",event=>{const{data}=event;setModvData(JSON.parse(data));const now=new Date;window.dispatchEvent(new CustomEvent("received-data-from-modv",{detail:{now}}))})}render(){const{connected}=this,connectedLabel=connected?"disconnect":"connect";return html$1`
      modV: <button @click="${e=>this.handleClick(e)}">${connectedLabel}</button>
    `}}customElements.define("modv-manager",ModvManager);class FivetwelveManager extends connect(store)(LitElement){static get properties(){return{connected:{type:Boolean},url:{type:String}}}_stateChanged(state){this.connected=getFivetwelveConnected(state);this.universes=getUniverses(state)}connectedCallback(){super.connectedCallback();this.url="ws://localhost:1234/";if(this.connected){this.createWebsocket()}window.addEventListener("send-universe-to-fivetwelve",this.listenSendUniverse.bind(this))}disconnectedCallback(){super.disconnectedCallback();this.disconnect();window.removeEventListener("send-universe-to-fivetwelve",this.listenSendUniverse.bind(this))}listenSendUniverse(){this.handleSend()}disconnect(){if(this.connected){this.socket.close();store.dispatch(connectFivetwelve(!1))}}handleClick(){if(this.connected){this.disconnect()}else{this.createWebsocket()}}handleSend(){this.send(0)}createWebsocket(){this.socket=new WebSocket(this.url);this.socket.binaryType="arraybuffer";this.socket.addEventListener("open",()=>{console.info("fivetwelve WebSocket opened to",this.url);store.dispatch(connectFivetwelve(!0))});this.socket.addEventListener("close",event=>{console.info("fivetwelve WebSocket closed:",event);store.dispatch(connectFivetwelve(!1))});this.socket.addEventListener("error",error=>{console.error("fivetwelve WebSocket error:",error);store.dispatch(connectFivetwelve(!1))});this.socket.addEventListener("message",event=>{const{data}=event;console.log("fivetwelve WebSocket message:",event)})}send(universe){if(this.connected&&this.socket!==void 0){const batch=this.universes[0].channels,messageBuffer=new window.buffer.Buffer(3*batch.length);for(let channel=0,channelNo;channel<batch.length;channel++){channelNo=universe+1<<9|511&channel;messageBuffer.writeUInt16BE(channelNo,3*channel);messageBuffer.writeUInt8(batch[channel],3*channel+2)}this.socket.send(messageBuffer)}}render(){const{connected}=this,connectedLabel=connected?"disconnect":"connect";return html$1`
      fivetwelve: <button @click="${e=>this.handleClick(e)}">${connectedLabel}</button>

      <button @click="${e=>this.handleSend(e)}">Send</button>
    `}}customElements.define("fivetwelve-manager",FivetwelveManager);class DekkManager extends connect(store)(LitElement){static get properties(){return{connected:{type:Boolean},data:{type:Object},url:{type:String}}}_stateChanged(state){this.connected=getDekkConnected(state);this.data=getDekkData(state)}connectedCallback(){super.connectedCallback();this.url="ws://localhost:3001/luminave";if(this.connected){this.createWebsocket()}}handleClick(){if(this.connected){this.socket.close();store.dispatch(connectDekk(!1))}else{this.createWebsocket()}}createWebsocket(){this.socket=new WebSocket(this.url);this.socket.addEventListener("open",()=>{console.info("Dekk WebSocket opened to",this.url);store.dispatch(connectDekk(!0))});this.socket.addEventListener("close",event=>{console.info("Dekk WebSocket closed:",event);store.dispatch(connectDekk(!1))});this.socket.addEventListener("error",error=>{console.error("Dekk WebSocket error:",error);store.dispatch(connectDekk(!1))});this.socket.addEventListener("message",event=>{const{data}=event;this.changeScenes(this.data.scenes,"remove");store.dispatch(setDekkData({scenes:JSON.parse(data)}));this.changeScenes(this.data.scenes,"add")})}changeScenes(sceneNames,action){const arrayType=Array.isArray(sceneNames)?sceneNames:[sceneNames];arrayType.map(name=>{const scene=getSceneByName(store.getState(),{name});if(scene===void 0){console.log(`Scene "${name}" doesn't exist`)}else{switch(action){case"remove":store.dispatch(removeSceneFromTimelineAndResetFixtures(scene.id));break;case"add":store.dispatch(addSceneToTimeline(scene.id));break;default:}}})}render(){const{connected}=this,connectedLabel=connected?"disconnect":"connect";return html$1`
      Dekk: <button @click="${e=>this.handleClick(e)}">${connectedLabel}</button>
    `}}customElements.define("dekk-manager",DekkManager);class LuminaveStatusIndicator extends LitElement{static get properties(){return{status:{type:Boolean}}}computeClass(status){const statusClass=status?"active":"inactive";return statusClass}render(){const{status}=this;return html$1`
        <style>
          .active {
    ...
    `}}customElements.define("luminave-status-indicator",LuminaveStatusIndicator);class ColorGrid extends LitElement{static get properties(){return{rows:{type:Number},colors:{type:Array}}}computeGridVars(rows){const vars={"--rows":rows};return Object.keys(vars).map(key=>[key,vars[key]].join(":")).join(";")}computeItemVars(color){const vars={"--red":color[0],"--green":color[1],"--blue":color[2]};return Object.keys(vars).map(key=>[key,vars[key]].join(":")).join(";")}_toColorArray(colors){const colorChunks=[];let i=0;while(i<colors.length){const chunk={color:colors.slice(i,i+3)};colorChunks.push(chunk);i+=3}return colorChunks}computeLabel(index){return`${index+1}`}render(){const{rows,colors}=this;return html$1`
      ...
      <div class="container" style="${this.computeGridVars(rows)}">
        ...
        ${repeat(this._toColorArray(colors),element=>element,(element,index)=>html$1`
    ...
    `}}customElements.define("color-grid",ColorGrid);class UiSpacer extends LitElement{constructor(){super();this.height="1em"}static get properties(){return{height:{type:String}}}computeVars(height){const vars={"--height":height};return Object.keys(vars).map(key=>[key,vars[key]].join(":")).join(";")}render(){const{height}=this;return html$1`
      <style>
        div {
          width: 100%;
          height: var(--height);
    ...
    `}}customElements.define("ui-spacer",UiSpacer);class LuminaveStatus extends connect(store)(LitElement){static get properties(){return{usbConnected:{type:Boolean},modvConnected:{type:Boolean},fivetwelveConnected:{type:Boolean}}}_stateChanged(state){this.usbConnected=getUsbDmxControllerConnected(state);this.modvConnected=getModvConnected(state);this.fivetwelveConnected=getFivetwelveConnected(state)}render(){const{usbConnected,modvConnected,fivetwelveConnected}=this;return html$1`
      <style>
        .grid {
          display: flex;
          flex-direction: row;
    ...
    `}}customElements.define("luminave-status",LuminaveStatus);class TapButton extends LitElement{constructor(){super();this.delay=2e3;this.items=4;this.bpm=0;this.arr=[];this.date={then:new Date,now:new Date};this.timer=setTimeout(()=>{},0);this.ticking=!1;this.average=0}static get properties(){return{delay:{type:Number},items:{type:Number}}}run(){clearTimeout(this.timer);this.timer=setTimeout(()=>{this.ticking=!1;this.arr=[]},this.delay);this.date.then=this.date.now;this.date.now=new Date;this.diff=this.date.now-this.date.then;if(this.ticking){this.arr.push(this.diff);if(this.arr.length>this.items){const diffs=this.arr.reduce((result,t)=>result+=t);this.arr.shift();this.average=diffs/this.arr.length;this.bpm=~~(6e4/this.average);store.dispatch(setBpm(this.bpm))}}else{this.ticking=!0}}render(){return html$1`
      <button @click="${()=>this.run()}">TAP</button>
    `}}customElements.define("tap-button",TapButton);var index$13={TapButton:TapButton};class BPMMeter extends LitElement{static get properties(){return{bpm:{type:Number}}}render(){const{bpm}=this;return html$1`
      <div>${bpm}</div>
    `}}customElements.define("bpm-meter",BPMMeter);class StorageManager extends connect(store)(LitElement){static get properties(){return{live:{type:Boolean}}}_stateChanged(state){this.live=getLive(state)}firstUpdated(){store.subscribe(()=>{if(!this.live){localStorage.setItem(STORAGE_STATE,JSON.stringify(store.getState()))}})}resetStorage(){localStorage.removeItem(STORAGE_STATE);window.location.reload(!0)}printStorage(){console.log(store.getState())}downloadStorage(){return`data:text/json;charset=utf-8,${encodeURIComponent(localStorage.getItem(STORAGE_STATE))}`}render(){return html$1`
      <!--<button on-click="resetStorage">Reset storage</button>
      <button on-click="printStorage">Print storage</button>-->
    `}}customElements.define("storage-manager",StorageManager);class LiveMode extends connect(store)(LitElement){static get properties(){return{live:{type:Boolean}}}_stateChanged(state){this.live=getLive(state)}handleLive(){store.dispatch(setLive(!this.live))}render(){const liveLabel=this.live?"Live":"Edit";return html$1`
      <div>
        Mode: <button @click="${e=>this.handleLive(e)}">${liveLabel}</button>
      </div>
    `}}customElements.define("live-mode",LiveMode);class RainbowText extends LitElement{constructor(){super();this.duration="12s"}static get properties(){return{text:{type:String},duration:{type:String}}}computeText(text){return text.split("")}computeVars(duration,rainbowChars,index){const vars={"--length":rainbowChars.length,"--delay":index,"--duration":duration};return Object.keys(vars).map(key=>[key,vars[key]].join(":")).join(";")}render(){const{text,duration}=this,rainbowChars=this.computeText(text);return html$1`
      <style>
        :root {
          --duration: 1s;
          --length: 0;
      ...
      ${repeat(rainbowChars,char=>char,(char,index)=>html$1`<span style="${this.computeVars(duration,rainbowChars,index)}">${char}</span>`)}
    `}}customElements.define("rainbow-text",RainbowText);class LuminaveMenu extends connect(store)(LitElement){static get properties(){return{bpm:{type:Number},live:{type:Boolean},drawerOpened:{type:Boolean}}}_stateChanged(state){this.bpm=getBpm(state);this.live=getLive(state);this.drawerOpened=getDrawerOpened(state)}handleDrawer(opened){if(opened!==this.drawerOpened){store.dispatch(updateDrawerState(opened))}}render(){const{bpm,live,drawerOpened}=this;return html$1`
      <style>
        .grid {
          display: flex;
          flex-direction: column;
        ...
        <app-toolbar>
          <div main-title>
            <h1>
              ...
              ${live?html$1`<span class="live" align="left">live</span>`:""}
            </h1>
          </div>

          <!--<luminave-status></luminave-status>-->

          <paper-icon-button icon="menu" @click="${()=>this.handleDrawer(!0)}"></paper-icon-button>
        </app-toolbar>
      </app-header>
    ...
    `}}customElements.define("luminave-menu",LuminaveMenu);class TimelineAnimation extends LitElement{static get properties(){return{duration:{type:Number},progress:{type:Number},fixtureIds:{type:Array},animation:{type:Object},timeline:{type:Object}}}computeTimeline(keyframes){keyframes={...keyframes};const keyframesArray=this._toArray(keyframes);let properties=[];for(const step in keyframes){properties=properties.concat(Object.keys(keyframes[step]))}properties=[...new Set(properties)];const timeline=properties.map(name=>{const property$$1={name,keyframes:[],keyframes:keyframesArray.filter(keyframe=>keyframe.value[name]!==void 0).map(keyframe=>{return{time:keyframe.time,value:keyframe.value[name]}})};return property$$1});return new keytime(timeline)}_toArray(object){const array=[];for(const step in object){array.push({time:parseFloat(step),value:object[step]})}return array.sort((a,b)=>a.time-b.time)}computeProgress(){let progress=this.progress/this.duration;if(1<progress){progress=1}return progress}shouldUpdate(changedProperties){if(changedProperties.has("animation")){this.timeline=this.computeTimeline(this.animation.keyframes)}return!0}render(){if(this.fixtureIds===void 0){return}const interpolatedProperties=this.timeline.values(this.computeProgress());for(let i=0;i<this.fixtureIds.length;i++){addToFixtureBatch(this.fixtureIds[i],interpolatedProperties)}return html$1``}}customElements.define("timeline-animation",TimelineAnimation);class TimelineScene extends connect(store)(LitElement){static get properties(){return{scene:{type:Object},progress:{type:Number},animations:{type:Array}}}_stateChanged(state){if(!Object.is(this.animations,getAnimations(state))){this.animations=getAnimations(state);this.requestUpdate()}}render(){const{scene,progress}=this;return html$1`
      <style>
        h3 {
          font-size: 1em;
          font-weight: normal;
      ...
      <div>
        ...
        ${repeat(this.scene.animations,animationId=>html$1`
      ...
      <div class="timeline">
        ...
        <div class="scenes">
          ...
          <div class="grid">
            ...
            ${repeat(this.timelineScenes,scene=>html$1`
              <div class="item">
    ...
    `}}customElements.define("timeline-manager",TimelineManager);const tabs=html$1`
<style>
    paper-tabs {
        display: inline-block;
        background-color: var(--dark-primary-color);
...
`;var tabs$1={tabs:tabs};class LuminaveDashboard extends connect(store)(LitElement){static get properties(){return{live:{type:Boolean},_page:{type:String}}}firstUpdated(){installRouter(location=>store.dispatch(navigate(window.decodeURIComponent(location.pathname))))}_stateChanged(state){this._page=state.app.page}render(){return html$1`
      ${tabs}
      ...
      <main role="main" class="main-content">
        <universe-view ?active="${"universe"===this._page}" class="page"></universe-view>
        <midi-view ?active="${"midi"===this._page}" class="page"></midi-view>
        <scene-view ?active="${"scene"===this._page}" class="page"></scene-view>
        <animation-view ?active="${"animation"===this._page}" class="page"></animation-view>
        <fixture-view ?active="${"fixture"===this._page}" class="page"></fixture-view>
        <modv-view ?active="${"modv"===this._page}" class="page"></modv-view>
        <my-view404 ?active="${"view404"===this._page}" class="page"></my-view404>
      </main>
    `}}customElements.define("luminave-dashboard",LuminaveDashboard);const theme=html$1`
<style>
    :host {
        /* Luminave Custom */
        --padding-basic: .5em;
...
`;var theme$1={theme:theme};class Luminave extends LitElement{render(){return html$1`
      ${theme}
    ...
    `}}customElements.define("lumi-nave",Luminave);const shared=html$1`
<style>
    .items {
        display: flex;
        flex-wrap: wrap;
...
`;var shared$1={shared:shared};class PageViewElement extends LitElement{shouldUpdate(){return this.active}static get properties(){return{active:{type:Boolean}}}}var pageViewElement={PageViewElement:PageViewElement};export{UPDATE_PAGE,UPDATE_OFFLINE,UPDATE_DRAWER_STATE,OPEN_SNACKBAR,CLOSE_SNACKBAR,navigate,showSnackbar,updateOffline,updateLayout,updateDrawerState};