import{html$1 as html,Polymer,dom,LitElement,html as html$1,repeat,store,setChannels,setFixtureAddress,addFixture,removeFixtureFromEverywhere,$dmxFixtures as Fixtures,batch,connect,uuidV1,FIXTURE_TYPES,getFixtures,PageViewElement}from"../app.js";const template=html`<iron-iconset-svg name="maps" size="24">
<svg><defs>
...
</iron-iconset-svg>`;document.head.appendChild(template.content);Polymer({_template:html`
    <style>
      :host {
        display: block;
        position: absolute;
...
`,is:"paper-tooltip",hostAttributes:{role:"tooltip",tabindex:-1},properties:{for:{type:String,observer:"_findTarget"},manualMode:{type:Boolean,value:!1,observer:"_manualModeChanged"},position:{type:String,value:"bottom"},fitToVisibleBounds:{type:Boolean,value:!1},offset:{type:Number,value:14},marginTop:{type:Number,value:14},animationDelay:{type:Number,value:500,observer:"_delayChange"},animationEntry:{type:String,value:""},animationExit:{type:String,value:""},animationConfig:{type:Object,value:function(){return{entry:[{name:"fade-in-animation",node:this,timing:{delay:0}}],exit:[{name:"fade-out-animation",node:this}]}}},_showing:{type:Boolean,value:!1}},listeners:{webkitAnimationEnd:"_onAnimationEnd"},get target(){var parentNode=dom(this).parentNode,ownerRoot=dom(this).getOwnerRoot(),target;if(this.for){target=dom(ownerRoot).querySelector("#"+this.for)}else{target=parentNode.nodeType==Node.DOCUMENT_FRAGMENT_NODE?ownerRoot.host:parentNode}return target},attached:function(){this._findTarget()},detached:function(){if(!this.manualMode)this._removeListeners()},playAnimation:function(type){if("entry"===type){this.show()}else if("exit"===type){this.hide()}},cancelAnimation:function(){this.$.tooltip.classList.add("cancel-animation")},show:function(){if(this._showing)return;if(""===dom(this).textContent.trim()){for(var allChildrenEmpty=!0,effectiveChildren=dom(this).getEffectiveChildNodes(),i=0;i<effectiveChildren.length;i++){if(""!==effectiveChildren[i].textContent.trim()){allChildrenEmpty=!1;break}}if(allChildrenEmpty){return}}this._showing=!0;this.$.tooltip.classList.remove("hidden");this.$.tooltip.classList.remove("cancel-animation");this.$.tooltip.classList.remove(this._getAnimationType("exit"));this.updatePosition();this._animationPlaying=!0;this.$.tooltip.classList.add(this._getAnimationType("entry"))},hide:function(){if(!this._showing){return}if(this._animationPlaying){this._showing=!1;this._cancelAnimation();return}else{this._onAnimationFinish()}this._showing=!1;this._animationPlaying=!0},updatePosition:function(){var _Mathmax=Math.max;if(!this._target||!this.offsetParent)return;var offset=this.offset;if(14!=this.marginTop&&14==this.offset)offset=this.marginTop;var parentRect=this.offsetParent.getBoundingClientRect(),targetRect=this._target.getBoundingClientRect(),thisRect=this.getBoundingClientRect(),horizontalCenterOffset=(targetRect.width-thisRect.width)/2,verticalCenterOffset=(targetRect.height-thisRect.height)/2,targetLeft=targetRect.left-parentRect.left,targetTop=targetRect.top-parentRect.top,tooltipLeft,tooltipTop;switch(this.position){case"top":tooltipLeft=targetLeft+horizontalCenterOffset;tooltipTop=targetTop-thisRect.height-offset;break;case"bottom":tooltipLeft=targetLeft+horizontalCenterOffset;tooltipTop=targetTop+targetRect.height+offset;break;case"left":tooltipLeft=targetLeft-thisRect.width-offset;tooltipTop=targetTop+verticalCenterOffset;break;case"right":tooltipLeft=targetLeft+targetRect.width+offset;tooltipTop=targetTop+verticalCenterOffset;break;}if(this.fitToVisibleBounds){if(parentRect.left+tooltipLeft+thisRect.width>window.innerWidth){this.style.right="0px";this.style.left="auto"}else{this.style.left=_Mathmax(0,tooltipLeft)+"px";this.style.right="auto"}if(parentRect.top+tooltipTop+thisRect.height>window.innerHeight){this.style.bottom=parentRect.height+"px";this.style.top="auto"}else{this.style.top=_Mathmax(-parentRect.top,tooltipTop)+"px";this.style.bottom="auto"}}else{this.style.left=tooltipLeft+"px";this.style.top=tooltipTop+"px"}},_addListeners:function(){if(this._target){this.listen(this._target,"mouseenter","show");this.listen(this._target,"focus","show");this.listen(this._target,"mouseleave","hide");this.listen(this._target,"blur","hide");this.listen(this._target,"tap","hide")}this.listen(this.$.tooltip,"animationend","_onAnimationEnd");this.listen(this,"mouseenter","hide")},_findTarget:function(){if(!this.manualMode)this._removeListeners();this._target=this.target;if(!this.manualMode)this._addListeners()},_delayChange:function(newValue){if(500!==newValue){this.updateStyles({"--paper-tooltip-delay-in":newValue+"ms"})}},_manualModeChanged:function(){if(this.manualMode)this._removeListeners();else this._addListeners()},_cancelAnimation:function(){this.$.tooltip.classList.remove(this._getAnimationType("entry"));this.$.tooltip.classList.remove(this._getAnimationType("exit"));this.$.tooltip.classList.remove("cancel-animation");this.$.tooltip.classList.add("hidden")},_onAnimationFinish:function(){if(this._showing){this.$.tooltip.classList.remove(this._getAnimationType("entry"));this.$.tooltip.classList.remove("cancel-animation");this.$.tooltip.classList.add(this._getAnimationType("exit"))}},_onAnimationEnd:function(){this._animationPlaying=!1;if(!this._showing){this.$.tooltip.classList.remove(this._getAnimationType("exit"));this.$.tooltip.classList.add("hidden")}},_getAnimationType:function(type){if("entry"===type&&""!==this.animationEntry){return this.animationEntry}if("exit"===type&&""!==this.animationExit){return this.animationExit}if(this.animationConfig[type]&&"string"===typeof this.animationConfig[type][0].name){if(this.animationConfig[type][0].timing&&this.animationConfig[type][0].timing.delay&&0!==this.animationConfig[type][0].timing.delay){var timingDelay=this.animationConfig[type][0].timing.delay;if("entry"===type){this.updateStyles({"--paper-tooltip-delay-in":timingDelay+"ms"})}else if("exit"===type){this.updateStyles({"--paper-tooltip-delay-out":timingDelay+"ms"})}}return this.animationConfig[type][0].name}},_removeListeners:function(){if(this._target){this.unlisten(this._target,"mouseenter","show");this.unlisten(this._target,"focus","show");this.unlisten(this._target,"mouseleave","hide");this.unlisten(this._target,"blur","hide");this.unlisten(this._target,"tap","hide")}this.unlisten(this.$.tooltip,"animationend","_onAnimationEnd");this.unlisten(this,"mouseenter","hide")}});class DmxFixtureProperty extends LitElement{static get properties(){return{name:{type:String},type:{type:String},channels:{type:Number},property:{type:Object}}}handleColorChange(e){const value=e.target.value.match(/[A-Za-z0-9]{2}/g).map(v=>parseInt(v,16));this.dispatchEvent(new CustomEvent("change",{detail:{value,name:this.name}}))}handleInputChange(e){let{value}=e.target;if(!isNaN(value)){value=parseInt(value,10)}this.dispatchEvent(new CustomEvent("change",{detail:{value,name:this.name}}))}handleSelectChange(e){const[selectedOption]=e.target.selectedOptions,{value}=selectedOption;this.dispatchEvent(new CustomEvent("change",{detail:{value,name:this.name}}))}render(){const{channels,property,name}=this;return html$1`
    ...
    `}}customElements.define("dmx-fixture-property",DmxFixtureProperty);class DmxFixture extends LitElement{static get properties(){return{name:{type:String},id:{type:String},type:{type:String},address:{type:Number},universe:{type:Number},_fixture:{type:Object},_properties:{type:Object}}}handleSubmit(e){e.preventDefault();const data=new FormData(e.target),address=parseInt(data.get("address"),10);store.dispatch(setFixtureAddress(this.id,address))}handleChange(e){const{name,value}=e.detail;this._fixture[name]=value;store.dispatch(setChannels(0,[...batch]));const now=new Date;window.dispatchEvent(new CustomEvent("send-universe-to-usb-dmx-controller",{detail:{now}}));window.dispatchEvent(new CustomEvent("send-universe-to-fivetwelve",{detail:{now}}))}render(){this._fixture=new Fixtures[this.type]({address:this.address,universe:this.universe});this._properties=this._fixture.getParamsList();const{type,address,_fixture,_properties}=this;return html$1`
        ...
        <div>
          ...
          <div class="grid">
            ...
            ${repeat(_properties,property=>html$1`
    ...
    `}}customElements.define("dmx-fixture",DmxFixture);class FixtureManager extends connect(store)(LitElement){constructor(){super();this.types=FIXTURE_TYPES;this.types.sort()}static get properties(){return{fixtures:{type:Array}}}_stateChanged(state){this.fixtures=getFixtures(state)}removeFixture(e){const{fixtureId}=e.target;store.dispatch(removeFixtureFromEverywhere(fixtureId))}handleSubmit(e){e.preventDefault();const data=new FormData(e.target),type=data.get("type"),name=data.get("name"),universe=0,address=parseInt(data.get("address"),10),amount=parseInt(data.get("amount"),10);if(isNaN(amount)){store.dispatch(addFixture({id:uuidV1(),type,name,universe,address,properties:{}}))}else{const bulkFixture=new Fixtures[type]({address,universe}),offset=bulkFixture.channels;for(let i=0;i<amount;i++){const bulkAddress=address+offset*i;store.dispatch(addFixture({id:uuidV1(),type,name:`${name}${bulkAddress}`,universe,address:bulkAddress,properties:{}}))}}}render(){const{types,fixtures}=this;return html$1`
    <style>
      :host {
        --width: 4;
      }
      ...
      .item::before {
        ...
        <div class="container">
          ...
          ${repeat(fixtures,fixture=>html$1`
    ...
    `}}customElements.define("fixture-manager",FixtureManager);class FixtureView extends PageViewElement{render(){return html$1`
      <section>
