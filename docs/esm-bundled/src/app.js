...
</iron-iconset-svg>`;document.head.appendChild($_documentContainer$1.content);const $_documentContainer$2=document.createElement("template");$_documentContainer$2.setAttribute("style","display: none;");$_documentContainer$2.innerHTML=`<iron-iconset-svg name="maps" size="24">
<svg><defs>
...
</dom-module>`;document.head.appendChild($_documentContainer$5.content);Polymer({is:"paper-button",behaviors:[PaperButtonBehavior],properties:{raised:{type:Boolean,reflectToAttribute:!0,value:!1,observer:"_calculateElevation"}},_calculateElevation:function(){if(!this.raised){this._setElevation(0)}else{PaperButtonBehaviorImpl._calculateElevation.apply(this)}}});const $_documentContainer$6=document.createElement("template");$_documentContainer$6.setAttribute("style","display: none;");$_documentContainer$6.innerHTML=`<custom-style>
  <style is="custom-style">
    html {
...
</custom-style>`;document.head.appendChild($_documentContainer$6.content);const $_documentContainer$7=document.createElement("template");$_documentContainer$7.setAttribute("style","display: none;");$_documentContainer$7.innerHTML=`<custom-style>
  <style is="custom-style">
    html {
      /*
...
</custom-style>`;document.head.appendChild($_documentContainer$7.content);const $_documentContainer$8=document.createElement("template");$_documentContainer$8.setAttribute("style","display: none;");$_documentContainer$8.innerHTML=`<dom-module id="paper-icon-button">
  <template strip-whitespace="">
    <style>
      :host {
...
</dom-module>`;document.head.appendChild($_documentContainer$8.content);Polymer({is:"paper-icon-button",hostAttributes:{role:"button",tabindex:"0"},behaviors:[PaperInkyFocusBehavior],properties:{src:{type:String},icon:{type:String},alt:{type:String,observer:"_altChanged"}},_altChanged:function(newValue,oldValue){var label=this.getAttribute("aria-label");if(!label||oldValue==label){this.setAttribute("aria-label",newValue)}}});Polymer({_template:html`
    <style>
      :host {
        @apply --layout-inline;
        @apply --layout-center;
...
`,is:"paper-tooltip",hostAttributes:{role:"tooltip",tabindex:-1},properties:{for:{type:String,observer:"_findTarget"},manualMode:{type:Boolean,value:!1,observer:"_manualModeChanged"},position:{type:String,value:"bottom"},fitToVisibleBounds:{type:Boolean,value:!1},offset:{type:Number,value:14},marginTop:{type:Number,value:14},animationDelay:{type:Number,value:500,observer:"_delayChange"},animationEntry:{type:String,value:""},animationExit:{type:String,value:""},animationConfig:{type:Object,value:function(){return{entry:[{name:"fade-in-animation",node:this,timing:{delay:0}}],exit:[{name:"fade-out-animation",node:this}]}}},_showing:{type:Boolean,value:!1}},listeners:{webkitAnimationEnd:"_onAnimationEnd"},get target(){var parentNode=dom(this).parentNode,ownerRoot=dom(this).getOwnerRoot(),target;if(this.for){target=dom(ownerRoot).querySelector("#"+this.for)}else{target=parentNode.nodeType==Node.DOCUMENT_FRAGMENT_NODE?ownerRoot.host:parentNode}return target},attached:function(){this._findTarget()},detached:function(){if(!this.manualMode)this._removeListeners()},playAnimation:function(type){if("entry"===type){this.show()}else if("exit"===type){this.hide()}},cancelAnimation:function(){this.$.tooltip.classList.add("cancel-animation")},show:function(){if(this._showing)return;if(""===dom(this).textContent.trim()){for(var allChildrenEmpty=!0,effectiveChildren=dom(this).getEffectiveChildNodes(),i=0;i<effectiveChildren.length;i++){if(""!==effectiveChildren[i].textContent.trim()){allChildrenEmpty=!1;break}}if(allChildrenEmpty){return}}this._showing=!0;this.$.tooltip.classList.remove("hidden");this.$.tooltip.classList.remove("cancel-animation");this.$.tooltip.classList.remove(this._getAnimationType("exit"));this.updatePosition();this._animationPlaying=!0;this.$.tooltip.classList.add(this._getAnimationType("entry"))},hide:function(){if(!this._showing){return}if(this._animationPlaying){this._showing=!1;this._cancelAnimation();return}else{this._onAnimationFinish()}this._showing=!1;this._animationPlaying=!0},updatePosition:function(){var _Mathmax3=Math.max;if(!this._target||!this.offsetParent)return;var offset=this.offset;if(14!=this.marginTop&&14==this.offset)offset=this.marginTop;var parentRect=this.offsetParent.getBoundingClientRect(),targetRect=this._target.getBoundingClientRect(),thisRect=this.getBoundingClientRect(),horizontalCenterOffset=(targetRect.width-thisRect.width)/2,verticalCenterOffset=(targetRect.height-thisRect.height)/2,targetLeft=targetRect.left-parentRect.left,targetTop=targetRect.top-parentRect.top,tooltipLeft,tooltipTop;switch(this.position){case"top":tooltipLeft=targetLeft+horizontalCenterOffset;tooltipTop=targetTop-thisRect.height-offset;break;case"bottom":tooltipLeft=targetLeft+horizontalCenterOffset;tooltipTop=targetTop+targetRect.height+offset;break;case"left":tooltipLeft=targetLeft-thisRect.width-offset;tooltipTop=targetTop+verticalCenterOffset;break;case"right":tooltipLeft=targetLeft+targetRect.width+offset;tooltipTop=targetTop+verticalCenterOffset;break;}if(this.fitToVisibleBounds){if(parentRect.left+tooltipLeft+thisRect.width>window.innerWidth){this.style.right="0px";this.style.left="auto"}else{this.style.left=_Mathmax3(0,tooltipLeft)+"px";this.style.right="auto"}if(parentRect.top+tooltipTop+thisRect.height>window.innerHeight){this.style.bottom=parentRect.height+"px";this.style.top="auto"}else{this.style.top=_Mathmax3(-parentRect.top,tooltipTop)+"px";this.style.bottom="auto"}}else{this.style.left=tooltipLeft+"px";this.style.top=tooltipTop+"px"}},_addListeners:function(){if(this._target){this.listen(this._target,"mouseenter","show");this.listen(this._target,"focus","show");this.listen(this._target,"mouseleave","hide");this.listen(this._target,"blur","hide");this.listen(this._target,"tap","hide")}this.listen(this.$.tooltip,"animationend","_onAnimationEnd");this.listen(this,"mouseenter","hide")},_findTarget:function(){if(!this.manualMode)this._removeListeners();this._target=this.target;if(!this.manualMode)this._addListeners()},_delayChange:function(newValue){if(500!==newValue){this.updateStyles({"--paper-tooltip-delay-in":newValue+"ms"})}},_manualModeChanged:function(){if(this.manualMode)this._removeListeners();else this._addListeners()},_cancelAnimation:function(){this.$.tooltip.classList.remove(this._getAnimationType("entry"));this.$.tooltip.classList.remove(this._getAnimationType("exit"));this.$.tooltip.classList.remove("cancel-animation");this.$.tooltip.classList.add("hidden")},_onAnimationFinish:function(){if(this._showing){this.$.tooltip.classList.remove(this._getAnimationType("entry"));this.$.tooltip.classList.remove("cancel-animation");this.$.tooltip.classList.add(this._getAnimationType("exit"))}},_onAnimationEnd:function(){this._animationPlaying=!1;if(!this._showing){this.$.tooltip.classList.remove(this._getAnimationType("exit"));this.$.tooltip.classList.add("hidden")}},_getAnimationType:function(type){if("entry"===type&&""!==this.animationEntry){return this.animationEntry}if("exit"===type&&""!==this.animationExit){return this.animationExit}if(this.animationConfig[type]&&"string"===typeof this.animationConfig[type][0].name){if(this.animationConfig[type][0].timing&&this.animationConfig[type][0].timing.delay&&0!==this.animationConfig[type][0].timing.delay){var timingDelay=this.animationConfig[type][0].timing.delay;if("entry"===type){this.updateStyles({"--paper-tooltip-delay-in":timingDelay+"ms"})}else if("exit"===type){this.updateStyles({"--paper-tooltip-delay-out":timingDelay+"ms"})}}return this.animationConfig[type][0].name}},_removeListeners:function(){if(this._target){this.unlisten(this._target,"mouseenter","show");this.unlisten(this._target,"focus","show");this.unlisten(this._target,"mouseleave","hide");this.unlisten(this._target,"blur","hide");this.unlisten(this._target,"tap","hide")}this.unlisten(this.$.tooltip,"animationend","_onAnimationEnd");this.unlisten(this,"mouseenter","hide")}});const invariant=(condition,format,a,b,c,d,e,f)=>{if(!condition){let error;if("undefined"===typeof format){error=new Error("Minified exception occurred use the non-minified dev environment "+"for the full error message and additional helpful warnings.")}else{const args=[a,b,c,d,e,f],argIndex=0;error=new Error(format.replace(/%s/g,()=>args[argIndex++]));error.name="Invariant Violation"}error.framesToPop=1;throw error}};var invariant$1={default:invariant};const{splice}=Array.prototype,{hasOwnProperty,toString}=Object.prototype,type=obj=>toString.call(obj).slice(8,-1),getAllKeys="function"===typeof Object.getOwnPropertySymbols?obj=>Object.keys(obj).concat(Object.getOwnPropertySymbols(obj)):obj=>Object.keys(obj);function copy(object){if(Array.isArray(object)){return Object.assign(object.constructor(object.length),object)}else if("Map"===type(object)){return new Map(object)}else if("Set"===type(object)){return new Set(object)}else if(object&&"object"===typeof object){const prototype=object.constructor&&object.constructor.prototype;return Object.assign(Object.create(prototype||null),object)}else{return object}}function newContext(){const commands=Object.assign({},defaultCommands);update.extend=function(directive,fn){commands[directive]=fn};update.isEquals=function(a,b){return a===b};return update;function update(object,spec){if(!(Array.isArray(object)&&Array.isArray(spec))){invariant(!Array.isArray(spec),"update(): You provided an invalid spec to update(). The spec may "+"not contain an array except as the value of $set, $push, $unshift, "+"$splice or any custom command allowing an array value.")}invariant("object"===typeof spec&&null!==spec,"update(): You provided an invalid spec to update(). The spec and "+"every included key path must be plain objects containing one of the "+"following commands: %s.",Object.keys(commands).join(", "));let nextObject=object;getAllKeys(spec).forEach(function(key){if(hasOwnProperty.call(commands,key)){const objectWasNextObject=object===nextObject;nextObject=commands[key](spec[key],nextObject,spec,object);if(objectWasNextObject&&update.isEquals(nextObject,object)){nextObject=object}}else{const nextValueForKey=update(object[key],spec[key]);if(!update.isEquals(nextValueForKey,nextObject[key])||"undefined"===typeof nextValueForKey&&!hasOwnProperty.call(object,key)){if(nextObject===object){nextObject=copy(object)}nextObject[key]=nextValueForKey}}});return nextObject}}const defaultCommands={$push:function(value,nextObject,spec){invariantPushAndUnshift(nextObject,spec,"$push");return value.length?nextObject.concat(value):nextObject},$unshift:function(value,nextObject,spec){invariantPushAndUnshift(nextObject,spec,"$unshift");return value.length?value.concat(nextObject):nextObject},$splice:function(value,nextObject,spec,originalObject){invariantSplices(nextObject,spec);value.forEach(function(args){invariantSplice(args);if(nextObject===originalObject&&args.length)nextObject=copy(originalObject);splice.apply(nextObject,args)});return nextObject},$set:function(value,nextObject,spec){invariantSet(spec);return value},$toggle:function(targets,nextObject){invariantSpecArray(targets,"$toggle");const nextObjectCopy=targets.length?copy(nextObject):nextObject;targets.forEach(function(target){nextObjectCopy[target]=!nextObject[target]});return nextObjectCopy},$unset:function(value,nextObject,spec,originalObject){invariantSpecArray(value,"$unset");value.forEach(function(key){if(Object.hasOwnProperty.call(nextObject,key)){if(nextObject===originalObject)nextObject=copy(originalObject);delete nextObject[key]}});return nextObject},$add:function(value,nextObject,spec,originalObject){invariantMapOrSet(nextObject,"$add");invariantSpecArray(value,"$add");if("Map"===type(nextObject)){value.forEach(function(pair){const[key,value]=pair;if(nextObject===originalObject&&nextObject.get(key)!==value)nextObject=copy(originalObject);nextObject.set(key,value)})}else{value.forEach(function(value){if(nextObject===originalObject&&!nextObject.has(value))nextObject=copy(originalObject);nextObject.add(value)})}return nextObject},$remove:function(value,nextObject,spec,originalObject){invariantMapOrSet(nextObject,"$remove");invariantSpecArray(value,"$remove");value.forEach(function(key){if(nextObject===originalObject&&nextObject.has(key))nextObject=copy(originalObject);nextObject.delete(key)});return nextObject},$merge:function(value,nextObject,spec,originalObject){invariantMerge(nextObject,value);getAllKeys(value).forEach(function(key){if(value[key]!==nextObject[key]){if(nextObject===originalObject)nextObject=copy(originalObject);nextObject[key]=value[key]}});return nextObject},$apply:function(value,original){invariantApply(value);return value(original)}};function invariantPushAndUnshift(value,spec,command){invariant(Array.isArray(value),"update(): expected target of %s to be an array got %s.",command,value);invariantSpecArray(spec[command],command)}function invariantSpecArray(spec,command){invariant(Array.isArray(spec),"update(): expected spec of %s to be an array got %s. "+"Did you forget to wrap your parameter in an array?",command,spec)}function invariantSplices(value,spec){invariant(Array.isArray(value),"Expected $splice target to be an array got %s",value);invariantSplice(spec.$splice)}function invariantSplice(value){invariant(Array.isArray(value),"update(): expected spec of $splice to be an array of arrays got %s. "+"Did you forget to wrap your parameters in an array?",value)}function invariantApply(fn){invariant("function"===typeof fn,"update(): expected spec of $apply to be a function got %s.",fn)}function invariantSet(spec){invariant(1===Object.keys(spec).length,"Cannot have more than one key in an object with $set")}function invariantMerge(target,specValue){invariant(specValue&&"object"===typeof specValue,"update(): $merge expects a spec of type 'object' got %s",specValue);invariant(target&&"object"===typeof target,"update(): $merge expects a target of type 'object' got %s",target)}function invariantMapOrSet(target,command){const typeOfTarget=type(target);invariant("Map"===typeOfTarget||"Set"===typeOfTarget,"update(): %s expects a target of type Set or Map got %s",command,typeOfTarget)}var update=newContext(),index$7={default:update,newContext:newContext};function PolymerRedux(store){if(!store){throw new TypeError("PolymerRedux: expecting a redux store.")}else if(!["getState","dispatch","subscribe"].every(k=>"function"===typeof store[k])){throw new TypeError("PolymerRedux: invalid store object.")}const subscribers=new Map,bind=(element,properties)=>{const bindings=Object.keys(properties).filter(name=>{const property=properties[name];if(Object.prototype.hasOwnProperty.call(property,"statePath")){if(!property.readOnly&&property.notify){console.warn(`PolymerRedux: <${element.constructor.is}>.${name} has "notify" enabled, two-way bindings goes against Redux's paradigm`)}return!0}return!1}),update=state=>{let propertiesChanged=!1;bindings.forEach(name=>{const{statePath}=properties[name],value="function"===typeof statePath?statePath.call(element,state):get(state,statePath),changed=element._setPendingPropertyOrPath(name,value,!0);propertiesChanged=propertiesChanged||changed});if(propertiesChanged){element._invalidateProperties()}},unsubscribe=store.subscribe(()=>{const detail=store.getState();update(detail)});subscribers.set(element,unsubscribe);update(store.getState());return update},unbind=element=>{const off=subscribers.get(element);if("function"===typeof off){off()}},collect=(what,which)=>{let res={};while(what){res=Object.assign({},what[which],res);what=Object.getPrototypeOf(what)}return res};return parent=>class extends parent{constructor(){super();const actions=collect(this.constructor,"actions");Object.defineProperty(this,"_reduxActions",{configurable:!0,value:actions})}connectedCallback(){super.connectedCallback();const properties=collect(this.constructor,"properties");bind(this,properties)}disconnectedCallback(){super.disconnectedCallback();unbind(this)}dispatch(...args){const actions=this._reduxActions;let[action]=args;if("string"===typeof action){if("function"!==typeof actions[action]){throw new TypeError(`PolymerRedux: <${this.constructor.is}> invalid action creator "${action}"`)}action=actions[action](...args.slice(1))}if("function"===typeof action){const originalAction=action;action=(...args)=>{args.splice(0,1,(...args)=>{return this.dispatch(...args)});return originalAction(...args)};Object.keys(originalAction).forEach(prop=>{action[prop]=originalAction[prop]})}return store.dispatch(action)}getState(){return store.getState()}}}var polymerRedux={default:PolymerRedux};function createThunkMiddleware(extraArgument){return({dispatch,getState})=>next=>action=>{if("function"===typeof action){return action(dispatch,getState,extraArgument)}return next(action)}}const thunk=createThunkMiddleware();thunk.withExtraArgument=createThunkMiddleware;var index$8={default:thunk};function defaultEqualityCheck(a,b){return a===b}function areArgumentsShallowlyEqual(equalityCheck,prev,next){if(null===prev||null===next||prev.length!==next.length){return!1}const length=prev.length;for(let i=0;i<length;i++){if(!equalityCheck(prev[i],next[i])){return!1}}return!0}function defaultMemoize(func,equalityCheck=defaultEqualityCheck){let lastArgs=null,lastResult=null;return function(){if(!areArgumentsShallowlyEqual(equalityCheck,lastArgs,arguments)){lastResult=func.apply(null,arguments)}lastArgs=arguments;return lastResult}}function getDependencies(funcs){const dependencies=Array.isArray(funcs[0])?funcs[0]:funcs;if(!dependencies.every(dep=>"function"===typeof dep)){const dependencyTypes=dependencies.map(dep=>typeof dep).join(", ");throw new Error("Selector creators expect all input-selectors to be functions, "+`instead received the following types: [${dependencyTypes}]`)}return dependencies}function createSelectorCreator(memoize,...memoizeOptions){return(...funcs)=>{let recomputations=0;const resultFunc=funcs.pop(),dependencies=getDependencies(funcs),memoizedResultFunc=memoize(function(){recomputations++;return resultFunc.apply(null,arguments)},...memoizeOptions),selector=defaultMemoize(function(){const params=[],length=dependencies.length;for(let i=0;i<length;i++){params.push(dependencies[i].apply(null,arguments))}return memoizedResultFunc.apply(null,params)});selector.resultFunc=resultFunc;selector.recomputations=()=>recomputations;selector.resetRecomputations=()=>recomputations=0;return selector}}const createSelector=createSelectorCreator(defaultMemoize);function createStructuredSelector(selectors,selectorCreator=createSelector){if("object"!==typeof selectors){throw new Error("createStructuredSelector expects first argument to be an object "+`where each property is a selector, instead received a ${typeof selectors}`)}const objectKeys=Object.keys(selectors);return selectorCreator(objectKeys.map(key=>selectors[key]),(...values)=>{return values.reduce((composition,value,index)=>{composition[objectKeys[index]]=value;return composition},{})})}var index$9={defaultMemoize:defaultMemoize,createSelectorCreator:createSelectorCreator,createSelector:createSelector,createStructuredSelector:createStructuredSelector};class Controller{constructor(args={}){this.device=args.device||void 0;this.filters=args.filters||[{vendorId:9025,productId:32822},{vendorId:9025,productId:54},{vendorId:10755,productId:32822},{vendorId:10755,productId:54},{vendorId:10755,productId:64},{vendorId:10755,productId:32832},{vendorId:10374,productId:32770}];this.universe=args.universe||Array(512).fill(0)}enable(){return navigator.usb.requestDevice({filters:this.filters}).then(selectedDevice=>{this.device=selectedDevice})}getPairedDevice(){return navigator.usb.getDevices().then(devices=>{return devices[0]})}autoConnect(){return this.getPairedDevice().then(device=>{this.device=device;return new Promise((resolve,reject)=>{if(this.device===void 0){return reject(new Error("Can not find USB device."))}else{return resolve(this.connect())}})})}connect(){return this.device.open().then(()=>{if(null===this.device.configuration){return this.device.selectConfiguration(1)}}).then(()=>this.device.claimInterface(2)).then(()=>this.device.controlTransferOut({requestType:"class",recipient:"interface",request:34,value:1,index:2})).catch(error=>console.log(error))}send(data){return new Promise((resolve,reject)=>{if(this.device===void 0){return reject(new Error("USB device is not connected to the computer"))}else{const buffer=Uint8Array.from(data);return resolve(this.device.transferOut(4,buffer))}})}updateUniverse(channel,value){return new Promise((resolve,reject)=>{channel=channel-1;if(Number.isInteger(value)){this.universe.splice(channel,1,value)}else if(Array.isArray(value)){this.universe.splice(channel,value.length,...value)}else{return reject(new Error("Could not update Universe because the provided value is not of type number or number[]"))}return resolve(this.send(this.universe))})}disconnect(){return this.device.controlTransferOut({requestType:"class",recipient:"interface",request:34,value:1,index:2}).then(()=>this.device.close())}}var controller={default:Controller};const SET_CHANNEL=uuid(),SET_CHANNELS=uuid(),GET_CHANNEL=uuid(),SET_BPM=uuid(),SET_LIVE=uuid(),CONNECT_USB=uuid(),CONNECT_BLUETOOTH=uuid(),ADD_UNIVERSE=uuid(),REMOVE_UNIVERSE=uuid(),DEMO_UNIVERSE=uuid(),ADD_SCENE=uuid(),RUN_SCENE=uuid(),REMOVE_SCENE=uuid(),SET_SCENE_NAME=uuid(),ADD_ANIMATION_TO_SCENE=uuid(),REMOVE_ANIMATION_FROM_SCENE=uuid(),ADD_FIXTURE_TO_SCENE=uuid(),REMOVE_FIXTURE_FROM_SCENE=uuid(),ADD_ANIMATION=uuid(),RUN_ANIMATION=uuid(),REMOVE_ANIMATION=uuid(),SET_ANIMATION_NAME=uuid(),ADD_KEYFRAME=uuid(),ADD_FIXTURE=uuid(),SET_FIXTURE_ADDRESS=uuid(),SET_FIXTURE_PROPERTIES=uuid(),SET_ALL_FIXTURE_PROPERTIES=uuid(),RESET_FIXTURE_PROPERTIES=uuid(),REMOVE_FIXTURE=uuid(),ENABLE_MIDI=uuid(),ADD_MIDI=uuid(),REMOVE_MIDI=uuid(),ADD_MIDI_MAPPING=uuid(),SET_MIDI_MAPPING_ACTIVE=uuid(),LEARN_MIDI=uuid(),ADD_SCENE_TO_MIDI=uuid(),ADD_SCENES_TO_MIDI=uuid(),REMOVE_SCENE_FROM_MIDI=uuid(),ADD_SCENE_TO_TIMELINE=uuid(),REMOVE_SCENE_FROM_TIMELINE=uuid(),PLAY_TIMELINE=uuid(),RESET_TIMELINE=uuid(),SET_TIMELINE_PROGRESS=uuid(),SEND_UNIVERSE_TO_USB=uuid(),SET_MODV_COLOR=uuid(),CONNECT_MODV=uuid(),CONNECT_DEKK=uuid(),SET_DEKK_DATA=uuid(),CONNECT_FIVETWELVE=uuid(),SEND_UNIVERSE_TO_FIVETWELVE=uuid(),STORAGE_STATE="LuminaveConfig",FIXTURE_TYPES=["EuroliteTMH8","FunGenerationSeParQuadLedRgbUv","AdjStarburst","CameoFlatPar1RGBW","CameoPixBar600PRO","CameoWookie200RGY","StairvilleAF150","StairvilleBowlBeam604LEDCOBMovingHead","JsFestMovingHeadWash","StairvilleOutdoorStageParTri","BasicColor","BasicFluter"],FIXTURE_PROPERTIES=["color","dimmer","strobe","white","yellow","uv","rotate","pan","tilt","speed","brightness","colorSpeed","movement","gobo","amount","mode","modvColor","colors","pattern","zoom","xAxisRolling","yAxisRolling","zAxisRolling","xAxisMoving","yAxisMoving","panTilt","panEndless","tiltEndless","sound","colorMacro","strobeMacro","scanSpeed","rgbwMacro","resetAuto"],MIDI_TYPE_BUTTON="Button",MIDI_TYPE_KNOB="Knob",MIDI_TYPE_FADER="Fader",MIDI_TYPE_EMPTY="Empty",MIDI_TYPES=[MIDI_TYPE_BUTTON,MIDI_TYPE_KNOB,MIDI_TYPE_FADER,MIDI_TYPE_EMPTY];var index$10={SET_CHANNEL:SET_CHANNEL,SET_CHANNELS:SET_CHANNELS,GET_CHANNEL:GET_CHANNEL,SET_BPM:SET_BPM,SET_LIVE:SET_LIVE,CONNECT_USB:CONNECT_USB,CONNECT_BLUETOOTH:CONNECT_BLUETOOTH,ADD_UNIVERSE:ADD_UNIVERSE,REMOVE_UNIVERSE:REMOVE_UNIVERSE,DEMO_UNIVERSE:DEMO_UNIVERSE,ADD_SCENE:ADD_SCENE,RUN_SCENE:RUN_SCENE,REMOVE_SCENE:REMOVE_SCENE,SET_SCENE_NAME:SET_SCENE_NAME,ADD_ANIMATION_TO_SCENE:ADD_ANIMATION_TO_SCENE,REMOVE_ANIMATION_FROM_SCENE:REMOVE_ANIMATION_FROM_SCENE,ADD_FIXTURE_TO_SCENE:ADD_FIXTURE_TO_SCENE,REMOVE_FIXTURE_FROM_SCENE:REMOVE_FIXTURE_FROM_SCENE,ADD_ANIMATION:ADD_ANIMATION,RUN_ANIMATION:RUN_ANIMATION,REMOVE_ANIMATION:REMOVE_ANIMATION,SET_ANIMATION_NAME:SET_ANIMATION_NAME,ADD_KEYFRAME:ADD_KEYFRAME,ADD_FIXTURE:ADD_FIXTURE,SET_FIXTURE_ADDRESS:SET_FIXTURE_ADDRESS,SET_FIXTURE_PROPERTIES:SET_FIXTURE_PROPERTIES,SET_ALL_FIXTURE_PROPERTIES:SET_ALL_FIXTURE_PROPERTIES,RESET_FIXTURE_PROPERTIES:RESET_FIXTURE_PROPERTIES,REMOVE_FIXTURE:REMOVE_FIXTURE,ENABLE_MIDI:ENABLE_MIDI,ADD_MIDI:ADD_MIDI,REMOVE_MIDI:REMOVE_MIDI,ADD_MIDI_MAPPING:ADD_MIDI_MAPPING,SET_MIDI_MAPPING_ACTIVE:SET_MIDI_MAPPING_ACTIVE,LEARN_MIDI:LEARN_MIDI,ADD_SCENE_TO_MIDI:ADD_SCENE_TO_MIDI,ADD_SCENES_TO_MIDI:ADD_SCENES_TO_MIDI,REMOVE_SCENE_FROM_MIDI:REMOVE_SCENE_FROM_MIDI,ADD_SCENE_TO_TIMELINE:ADD_SCENE_TO_TIMELINE,REMOVE_SCENE_FROM_TIMELINE:REMOVE_SCENE_FROM_TIMELINE,PLAY_TIMELINE:PLAY_TIMELINE,RESET_TIMELINE:RESET_TIMELINE,SET_TIMELINE_PROGRESS:SET_TIMELINE_PROGRESS,SEND_UNIVERSE_TO_USB:SEND_UNIVERSE_TO_USB,SET_MODV_COLOR:SET_MODV_COLOR,CONNECT_MODV:CONNECT_MODV,CONNECT_DEKK:CONNECT_DEKK,SET_DEKK_DATA:SET_DEKK_DATA,CONNECT_FIVETWELVE:CONNECT_FIVETWELVE,SEND_UNIVERSE_TO_FIVETWELVE:SEND_UNIVERSE_TO_FIVETWELVE,STORAGE_STATE:STORAGE_STATE,FIXTURE_TYPES:FIXTURE_TYPES,FIXTURE_PROPERTIES:FIXTURE_PROPERTIES,MIDI_TYPE_BUTTON:MIDI_TYPE_BUTTON,MIDI_TYPE_KNOB:MIDI_TYPE_KNOB,MIDI_TYPE_FADER:MIDI_TYPE_FADER,MIDI_TYPE_EMPTY:MIDI_TYPE_EMPTY,MIDI_TYPES:MIDI_TYPES};const batch=Array(512).fill(0),addToBatch=(channel,value)=>{batch[channel]=value},clearBatch=()=>{batch.fill(0)},fixtureBatch={},addToFixtureBatch=(fixtureId,properties)=>{fixtureBatch[fixtureId]=fixtureBatch[fixtureId]||{properties:{}};Object.assign(fixtureBatch[fixtureId].properties,properties)},clearFixtureBatch=()=>{Object.keys(fixtureBatch).forEach(key=>{delete fixtureBatch[key]})},clearFixtureInBatch=fixtureId=>{if(fixtureBatch[fixtureId]!==void 0){Reflect.deleteProperty(fixtureBatch,fixtureId)}},modvData={average:[0,0,0],colors:[]},setModvData=data=>{modvData.average=data.average;modvData.colors=data.colors},collator=new Intl.Collator("en",{numeric:!0,sensitivity:"base"});var index$11={batch:batch,addToBatch:addToBatch,clearBatch:clearBatch,fixtureBatch:fixtureBatch,addToFixtureBatch:addToFixtureBatch,clearFixtureBatch:clearFixtureBatch,clearFixtureInBatch:clearFixtureInBatch,modvData:modvData,setModvData:setModvData,collator:collator};const getScenes=state=>state.sceneManager,getAnimations=state=>state.animationManager,getFixtures=state=>state.fixtureManager,getTimeline=state=>state.timelineManager,getTimelineSceneIds=state=>state.timelineManager.scenes,getAnimation=(state,properties)=>{return getAnimations(state).filter(animation=>animation.id===properties.animationId)[0]},getAnimationsSorted=createSelector(getAnimations,animations=>animations.sort((a,b)=>collator.compare(a.name,b.name))),getScene=(state,properties)=>{return getScenes(state).filter(scene=>scene.id===properties.sceneId)[0]},getSceneByName=(state,properties)=>{return getScenes(state).filter(scene=>scene.name===properties.name)[0]},getScenesSorted=createSelector(getScenes,scenes=>scenes.sort((a,b)=>collator.compare(a.name,b.name))),getTimelineScenes=createSelector(getScenes,getTimelineSceneIds,(scenes,timelineSceneIds)=>{return scenes.filter(scene=>{return timelineSceneIds.includes(scene.id)})}),getFixture=(state,properties)=>{return getFixtures(state).filter(fixture=>fixture.id===properties.fixtureId)[0]};var index$12={getScenes:getScenes,getAnimations:getAnimations,getFixtures:getFixtures,getTimeline:getTimeline,getTimelineSceneIds:getTimelineSceneIds,getAnimation:getAnimation,getAnimationsSorted:getAnimationsSorted,getScene:getScene,getSceneByName:getSceneByName,getScenesSorted:getScenesSorted,getTimelineScenes:getTimelineScenes,getFixture:getFixture};const setChannel=(universeIndex,channelIndex,value)=>({universeIndex,channelIndex,value,type:SET_CHANNEL}),setChannels=(universeIndex,channels)=>({universeIndex,channels,type:SET_CHANNELS}),getChannel=(universeIndex,channelIndex)=>({universeIndex,channelIndex,type:GET_CHANNEL}),setBpm=value=>({value,type:SET_BPM}),connectUsb=connected=>({connected,type:CONNECT_USB}),connectBluetooth=connected=>({connected,type:CONNECT_BLUETOOTH}),addUniverse=universe=>({universe,type:ADD_UNIVERSE}),removeUniverse=universeIndex=>({universeIndex,type:REMOVE_UNIVERSE}),resetUniverseAndFixtures=universeIndex=>{return dispatch=>{dispatch(resetAllFixtures());dispatch(setChannels(universeIndex,[...batch]));clearBatch()}},addScene=scene=>({scene,type:ADD_SCENE}),runScene=sceneIndex=>({sceneIndex,type:RUN_SCENE}),removeScene=sceneIndex=>({sceneIndex,type:REMOVE_SCENE}),setSceneName=(sceneIndex,sceneName)=>({sceneIndex,sceneName,type:SET_SCENE_NAME}),addAnimationToScene=(sceneIndex,animationId)=>({sceneIndex,animationId,type:ADD_ANIMATION_TO_SCENE}),removeAnimationFromScene=(sceneIndex,animationIndex)=>({sceneIndex,animationIndex,type:REMOVE_ANIMATION_FROM_SCENE}),addFixtureToScene=(sceneIndex,fixtureId)=>({sceneIndex,fixtureId,type:ADD_FIXTURE_TO_SCENE}),addFixturesToScene=(sceneIndex,fixtureIds)=>{return dispatch=>{fixtureIds.map(fixtureId=>dispatch(addFixtureToScene(sceneIndex,fixtureId)))}},removeFixtureFromScene=(sceneId,fixtureIndex)=>({sceneId,fixtureIndex,type:REMOVE_FIXTURE_FROM_SCENE}),resetAllFixtures=()=>{return(dispatch,getState)=>{getFixtures(getState()).map(fixture=>{clearFixtureInBatch(fixture.id);dispatch(resetFixtureProperties(fixture.id))})}},addAnimation=animation=>({animation,type:ADD_ANIMATION}),runAnimation=animationIndex=>({animationIndex,type:RUN_ANIMATION}),removeAnimation=animationIndex=>({animationIndex,type:REMOVE_ANIMATION}),setAnimationName=(animationIndex,animationName)=>({animationIndex,animationName,type:SET_ANIMATION_NAME}),addKeyframe=(animationIndex,keyframeStep,keyframeProperty,keyframeValue)=>({animationIndex,keyframeStep,keyframeProperty,keyframeValue,type:ADD_KEYFRAME}),addFixture=fixture=>({fixture,type:ADD_FIXTURE}),setFixtureAddress=(fixtureId,fixtureAddress)=>({fixtureId,fixtureAddress,type:SET_FIXTURE_ADDRESS}),setFixtureProperties=(fixtureId,properties)=>({fixtureId,properties,type:SET_FIXTURE_PROPERTIES}),setAllFixtureProperties=fixtureBatch$$1=>({fixtureBatch:fixtureBatch$$1,type:SET_ALL_FIXTURE_PROPERTIES}),resetFixtureProperties=fixtureId=>({fixtureId,type:RESET_FIXTURE_PROPERTIES}),removeFixture=fixtureIndex=>({fixtureIndex,type:REMOVE_FIXTURE}),removeFixtureFromEverywhere=fixtureId=>{return(dispatch,getState)=>{const fixtureIndex=getFixtures(getState()).findIndex(fixture=>fixture.id===fixtureId);clearFixtureInBatch(fixtureId);getScenes(getState()).map(scene=>{if(-1<scene.fixtures.indexOf(fixtureId)){dispatch(removeFixtureFromScene(scene.id,scene.fixtures.indexOf(fixtureId)))}});dispatch(removeFixture(fixtureIndex))}},addMidi=controller=>({controller,type:ADD_MIDI}),removeMidi=controllerIndex=>({controllerIndex,type:REMOVE_MIDI}),enableMidi=enabled=>({enabled,type:ENABLE_MIDI}),learnMidi=mappingIndex=>({mappingIndex,type:LEARN_MIDI}),addMidiMapping=(controllerIndex,mappingIndex,mapping)=>({controllerIndex,mappingIndex,mapping,type:ADD_MIDI_MAPPING}),setMidiMappingActive=(controllerIndex,mappingIndex,active)=>({controllerIndex,mappingIndex,active,type:SET_MIDI_MAPPING_ACTIVE}),addSceneToMidi=(controllerIndex,mappingIndex,sceneId)=>({controllerIndex,mappingIndex,sceneId,type:ADD_SCENE_TO_MIDI}),addScenesToMidi=(controllerIndex,mappingIndex,sceneIds)=>({controllerIndex,mappingIndex,sceneIds,type:ADD_SCENES_TO_MIDI}),removeSceneFromMidi=(controllerIndex,mappingIndex,sceneIndex)=>({controllerIndex,mappingIndex,sceneIndex,type:REMOVE_SCENE_FROM_MIDI}),playTimeline=playing=>({playing,type:PLAY_TIMELINE}),setTimelineProgress=progress=>({progress,type:SET_TIMELINE_PROGRESS}),resetTimeline=()=>({type:RESET_TIMELINE}),addSceneToTimeline=sceneId=>({sceneId,type:ADD_SCENE_TO_TIMELINE}),removeSceneFromTimeline=sceneId=>({sceneId,type:REMOVE_SCENE_FROM_TIMELINE}),removeSceneFromTimelineAndResetFixtures=sceneId=>{return(dispatch,getState)=>{getScene(getState(),{sceneId}).fixtures.map(fixtureId=>{clearFixtureInBatch(fixtureId)});dispatch(removeSceneFromTimeline(sceneId))}},setLive=value=>({value,type:SET_LIVE}),sendUniverseToUsb=value=>({value,type:SEND_UNIVERSE_TO_USB}),sendUniverseToFivetwelve=value=>({value,type:SEND_UNIVERSE_TO_FIVETWELVE}),setModvColor=color=>({color,type:SET_MODV_COLOR}),connectModv=connected=>({connected,type:CONNECT_MODV}),setDekkData=data=>({data,type:SET_DEKK_DATA}),connectDekk=connected=>({connected,type:CONNECT_DEKK}),connectFivetwelve=connected=>({connected,type:CONNECT_FIVETWELVE});var index$13={setChannel:setChannel,setChannels:setChannels,getChannel:getChannel,setBpm:setBpm,connectUsb:connectUsb,connectBluetooth:connectBluetooth,addUniverse:addUniverse,removeUniverse:removeUniverse,resetUniverseAndFixtures:resetUniverseAndFixtures,addScene:addScene,runScene:runScene,removeScene:removeScene,setSceneName:setSceneName,addAnimationToScene:addAnimationToScene,removeAnimationFromScene:removeAnimationFromScene,addFixtureToScene:addFixtureToScene,addFixturesToScene:addFixturesToScene,removeFixtureFromScene:removeFixtureFromScene,resetAllFixtures:resetAllFixtures,addAnimation:addAnimation,runAnimation:runAnimation,removeAnimation:removeAnimation,setAnimationName:setAnimationName,addKeyframe:addKeyframe,addFixture:addFixture,setFixtureAddress:setFixtureAddress,setFixtureProperties:setFixtureProperties,setAllFixtureProperties:setAllFixtureProperties,resetFixtureProperties:resetFixtureProperties,removeFixture:removeFixture,removeFixtureFromEverywhere:removeFixtureFromEverywhere,addMidi:addMidi,removeMidi:removeMidi,enableMidi:enableMidi,learnMidi:learnMidi,addMidiMapping:addMidiMapping,setMidiMappingActive:setMidiMappingActive,addSceneToMidi:addSceneToMidi,addScenesToMidi:addScenesToMidi,removeSceneFromMidi:removeSceneFromMidi,playTimeline:playTimeline,setTimelineProgress:setTimelineProgress,resetTimeline:resetTimeline,addSceneToTimeline:addSceneToTimeline,removeSceneFromTimeline:removeSceneFromTimeline,removeSceneFromTimelineAndResetFixtures:removeSceneFromTimelineAndResetFixtures,setLive:setLive,sendUniverseToUsb:sendUniverseToUsb,sendUniverseToFivetwelve:sendUniverseToFivetwelve,setModvColor:setModvColor,connectModv:connectModv,setDekkData:setDekkData,connectDekk:connectDekk,connectFivetwelve:connectFivetwelve};const bpm=(state=130,{type,value})=>{switch(type){case SET_BPM:return value;default:return state;}},live=(state=!1,{type,value})=>{switch(type){case SET_LIVE:return update(state,{$set:value});default:return state;}},usbManager=(state={lastTransmission:0},{type,value})=>{switch(type){case SEND_UNIVERSE_TO_USB:return update(state,{lastTransmission:{$set:value}});default:return state;}},modvManager=(state={color:[0,0,0],connected:!1},{type,color,connected})=>{switch(type){case CONNECT_MODV:return update(state,{connected:{$set:connected}});case SET_MODV_COLOR:return update(state,{color:{$set:color}});default:return state;}},dekkManager=(state={data:{},connected:!1},{type,data,connected})=>{switch(type){case CONNECT_DEKK:return update(state,{connected:{$set:connected}});case SET_DEKK_DATA:return update(state,{data:{$set:data}});default:return state;}},fivetwelveManager=(state={connected:!1},{type,connected,value})=>{switch(type){case CONNECT_FIVETWELVE:return update(state,{connected:{$set:connected}});case SEND_UNIVERSE_TO_FIVETWELVE:return update(state,{lastTransmission:{$set:value}});default:return state;}},connectionManager=(state={usb:{connected:!1},bluetooth:{connected:!1}},{type,connected})=>{switch(type){case CONNECT_USB:return Object.assign({},state,{usb:{connected}});case CONNECT_BLUETOOTH:return Object.assign({},state,{bluetooth:{connected}});default:return state;}},universeManager=(state=[],{type,universe,universeIndex,channelIndex,value,channels})=>{switch(type){case ADD_UNIVERSE:return update(state,{$push:[universe]});case REMOVE_UNIVERSE:return update(state,{$splice:[[universeIndex,1]]});case SET_CHANNEL:return update(state,{[universeIndex]:{channels:{$splice:[[channelIndex,1,value]]}}});case SET_CHANNELS:return update(state,{[universeIndex]:{channels:{$set:channels}}});case GET_CHANNEL:return state;default:return state;}},sceneManager=(state=[],{type,scene,sceneIndex,sceneName,sceneId,animationId,animationIndex,fixtureId,fixtureIndex})=>{switch(type){case ADD_SCENE:return update(state,{$push:[scene]});case RUN_SCENE:return update(state,{[sceneIndex]:{isRunning:{$set:!0}}});case REMOVE_SCENE:return update(state,{$splice:[[sceneIndex,1]]});case SET_SCENE_NAME:return update(state,{[sceneIndex]:{name:{$set:sceneName}}});case ADD_ANIMATION_TO_SCENE:return update(state,{[sceneIndex]:{animations:{$push:[animationId]}}});case REMOVE_ANIMATION_FROM_SCENE:return update(state,{[sceneIndex]:{animations:{$splice:[[animationIndex,1]]}}});case ADD_FIXTURE_TO_SCENE:return update(state,{[sceneIndex]:{fixtures:{$push:[fixtureId]}}});case REMOVE_FIXTURE_FROM_SCENE:{const sceneIndex=state.findIndex(scene=>scene.id===sceneId);return update(state,{[sceneIndex]:{fixtures:{$splice:[[fixtureIndex,1]]}}})}default:return state;}},animationManager=(state=[],{type,animation,animationIndex,animationName,keyframeStep,keyframeProperty,keyframeValue})=>{switch(type){case ADD_ANIMATION:return update(state,{$push:[animation]});case SET_ANIMATION_NAME:return update(state,{[animationIndex]:{name:{$set:animationName}}});case ADD_KEYFRAME:{if(isNaN(keyframeValue)){if(keyframeValue.includes(",")){keyframeValue=keyframeValue.split(",").map(color=>parseInt(color))}else if(!(0!==keyframeValue.length)){console.warn(keyframeValue,"for",keyframeProperty,"is not handled")}}else{keyframeValue=parseInt(keyframeValue,10)}const oldSteps=state[animationIndex].keyframes[keyframeStep]||{},newStep={[keyframeProperty]:keyframeValue};return update(state,{[animationIndex]:{keyframes:{$merge:{[keyframeStep]:{...oldSteps,...newStep}}}}})}case RUN_ANIMATION:return update(state,{[animationIndex]:{isRunning:{$set:!0}}});case REMOVE_ANIMATION:return update(state,{$splice:[[animationIndex,1]]});default:return state;}},fixtureManager=(state=[],{type,fixture,fixtureIndex,fixtureId,fixtureAddress,properties,fixtureBatch:fixtureBatch$$1})=>{switch(type){case ADD_FIXTURE:return update(state,{$push:[fixture]});case SET_FIXTURE_ADDRESS:{const fixtureIndex=state.findIndex(fixture=>fixture.id===fixtureId);return update(state,{[fixtureIndex]:{address:{$set:fixtureAddress}}})}case SET_FIXTURE_PROPERTIES:{const fixtureIndex=state.findIndex(fixture=>fixture.id===fixtureId),oldProperties=state[fixtureIndex].properties;return update(state,{[fixtureIndex]:{properties:{$merge:{...oldProperties,...properties}}}})}case SET_ALL_FIXTURE_PROPERTIES:{return state.map((fixture,i)=>{if(fixtureBatch$$1[fixture.id]!==void 0){return update(state[i],{properties:{$merge:fixtureBatch$$1[fixture.id].properties}})}else{return state[i]}})}case RESET_FIXTURE_PROPERTIES:{clearFixtureInBatch(fixtureId);const fixtureIndex=state.findIndex(fixture=>fixture.id===fixtureId);return update(state,{[fixtureIndex]:{properties:{$set:{}}}})}case REMOVE_FIXTURE:return update(state,{$splice:[[fixtureIndex,1]]});default:return state;}},midiManager=(state={controllers:[],enabled:!1,learning:!1},{type,controller,controllerIndex,enabled,mapping,mappingIndex,sceneId,sceneIds,sceneIndex,active})=>{switch(type){case ENABLE_MIDI:return update(state,{enabled:{$set:enabled}});case ADD_MIDI:return update(state,{controllers:{$push:[controller]}});case ADD_MIDI_MAPPING:{const old=state.controllers[controllerIndex].mapping[mappingIndex]||{};return update(state,{controllers:{[controllerIndex]:{mapping:{$merge:{[mappingIndex]:{...old,...mapping}}}}}})}case SET_MIDI_MAPPING_ACTIVE:return update(state,{controllers:{[controllerIndex]:{mapping:{[mappingIndex]:{active:{$set:active}}}}}});case ADD_SCENE_TO_MIDI:return update(state,{controllers:{[controllerIndex]:{mapping:{[mappingIndex]:{scenes:{$push:[sceneId]}}}}}});case ADD_SCENES_TO_MIDI:{const oldSceneIds=state.controllers[controllerIndex].mapping[mappingIndex].scenes,newSceneIds=sceneIds.filter(sceneId=>!oldSceneIds.includes(sceneId));return update(state,{controllers:{[controllerIndex]:{mapping:{[mappingIndex]:{scenes:{$push:newSceneIds}}}}}})}case REMOVE_SCENE_FROM_MIDI:return update(state,{controllers:{[controllerIndex]:{mapping:{[mappingIndex]:{scenes:{$splice:[[sceneIndex,1]]}}}}}});case LEARN_MIDI:return update(state,{learning:{$set:mappingIndex}});case REMOVE_MIDI:return update(state,{controllers:{$splice:[[controllerIndex,1]]}});default:return state;}},timelineManager=(state={scenes:[],playing:!1,progress:0},{type,sceneId,playing,progress})=>{switch(type){case PLAY_TIMELINE:return update(state,{playing:{$set:playing}});case ADD_SCENE_TO_TIMELINE:return update(state,{scenes:{$push:[sceneId]}});case REMOVE_SCENE_FROM_TIMELINE:{const sceneIndex=state.scenes.indexOf(sceneId);return update(state,{scenes:{$splice:[[sceneIndex,1]]}})}case RESET_TIMELINE:return update(state,{scenes:{$set:[]}});case SET_TIMELINE_PROGRESS:return update(state,{progress:{$set:progress}});default:return state;}};var reducers={bpm:bpm,live:live,usbManager:usbManager,modvManager:modvManager,dekkManager:dekkManager,fivetwelveManager:fivetwelveManager,connectionManager:connectionManager,universeManager:universeManager,sceneManager:sceneManager,animationManager:animationManager,fixtureManager:fixtureManager,midiManager:midiManager,timelineManager:timelineManager};const store=createStore(combineReducers({...reducers}),localStorage.getItem(STORAGE_STATE)?JSON.parse(localStorage.getItem(STORAGE_STATE)):{},applyMiddleware(thunk)),reduxMixin=PolymerRedux(store);var reduxStore={store:store,reduxMixin:reduxMixin,default:reduxMixin};class UsbDmxManager extends reduxMixin(PolymerElement){constructor(){super();this.controller=new Controller({filters:[{vendorId:9025,productId:32822},{vendorId:9025,productId:32823},{vendorId:10755,productId:32832}]});this.controller.autoConnect().then(()=>{this.connect()}).catch(error=>{console.log(error)})}static get properties(){return{usbConnection:{type:Boolean,statePath:"connectionManager.usb"},universes:{type:Array,statePath:"universeManager"}}}connectedCallback(){super.connectedCallback();window.addEventListener("send-universe-to-usb-dmx-controller",this.listenSendUniverse.bind(this))}disconnectedCallback(){super.disconnectedCallback();window.removeEventListener("send-universe-to-usb-dmx-controller",this.listenSendUniverse.bind(this))}listenSendUniverse(){if(this.usbConnection.connected){this.controller.send(this.universes[0].channels).catch(error=>{console.error(error)})}}handleConnectButtonClick(){if(this.usbConnection.connected){if(this.controller.device===void 0){this.dispatch(connectUsb(!1))}else{this.controller.disconnect().then(()=>{this.dispatch(connectUsb(!1))},error=>{console.error(error)})}}else{this.enable()}}enable(){this.controller.enable().then(()=>{this.connect()}).catch(error=>{console.error(error)})}connect(){this.controller.connect().then(()=>{this.dispatch(connectUsb(!0))},error=>{this.dispatch(connectUsb(!1));console.error(error)})}static get template(){return html`
      <connect-button type="usb" label="USB" on-click="handleConnectButtonClick"></connect-button>
    `}}customElements.define("usb-dmx-manager",UsbDmxManager);class ModvManager extends reduxMixin(PolymerElement){static get properties(){return{live:{type:Boolean,statePath:"live"},editMode:{type:Boolean,computed:"computeEditMode(live)"},connected:{type:Boolean,statePath:"modvManager.connected"},url:String,connectedLabel:{type:String,computed:"computeConnectedLabel(connected)"},averageColor:Array,colors:Array}}computeEditMode(live){return!live}computeConnectedLabel(connected){return connected?"disconnect":"connect"}connectedCallback(){super.connectedCallback();this.url="ws://localhost:3000/luminave";if(this.connected){this.createWebsocket()}}handleClick(){if(this.connected){this.socket.close();this.dispatch(connectModv(!1))}else{this.createWebsocket()}}createWebsocket(){this.socket=new WebSocket(this.url);this.socket.addEventListener("open",()=>{console.info("modV WebSocket opened to",this.url);this.dispatch(connectModv(!0))});this.socket.addEventListener("close",event=>{console.info("modV WebSocket closed:",event);this.dispatch(connectModv(!1))});this.socket.addEventListener("error",error=>{console.error("modV WebSocket error:",error);this.dispatch(connectModv(!1))});this.socket.addEventListener("message",event=>{const{data}=event;setModvData(JSON.parse(data));this.averageColor=modvData.average;this.colors=modvData.colors;const now=new Date;window.dispatchEvent(new CustomEvent("received-data-from-modv",{detail:{now}}))})}static get template(){return html`
        modV: <button on-click="handleClick">[[connectedLabel]]</button>
    `}}customElements.define("modv-manager",ModvManager);class FivetwelveManager extends reduxMixin(PolymerElement){static get properties(){return{connected:{type:Boolean,statePath:"fivetwelveManager.connected"},url:String,connectedLabel:{type:String,computed:"computeConnectedLabel(connected)"}}}computeConnectedLabel(connected){return connected?"disconnect":"connect"}connectedCallback(){super.connectedCallback();this.url="ws://localhost:1234/";if(this.connected){this.createWebsocket()}window.addEventListener("send-universe-to-fivetwelve",this.listenSendUniverse.bind(this))}disconnectedCallback(){super.disconnectedCallback();this.disconnect();window.removeEventListener("send-universe-to-fivetwelve",this.listenSendUniverse.bind(this))}listenSendUniverse(){this.handleSend()}disconnect(){if(this.connected){this.socket.close();this.dispatch(connectFivetwelve(!1))}}handleClick(){if(this.connected){this.disconnect()}else{this.createWebsocket()}}handleSend(){this.send(0)}createWebsocket(){this.socket=new WebSocket(this.url);this.socket.binaryType="arraybuffer";this.socket.addEventListener("open",()=>{console.info("fivetwelve WebSocket opened to",this.url);this.dispatch(connectFivetwelve(!0))});this.socket.addEventListener("close",event=>{console.info("fivetwelve WebSocket closed:",event);this.dispatch(connectFivetwelve(!1))});this.socket.addEventListener("error",error=>{console.error("fivetwelve WebSocket error:",error);this.dispatch(connectFivetwelve(!1))});this.socket.addEventListener("message",event=>{const{data}=event;console.log("fivetwelve WebSocket message:",event)})}send(universe){if(this.connected&&this.socket!==void 0){const messageBuffer=new window.buffer.Buffer(3*batch.length);for(let channel=0,channelNo;channel<batch.length;channel++){channelNo=universe+1<<9|511&channel;messageBuffer.writeUInt16BE(channelNo,3*channel);messageBuffer.writeUInt8(batch[channel],3*channel+2)}this.socket.send(messageBuffer)}}static get template(){return html`
      fivetwelve: <button on-click="handleClick">[[connectedLabel]]</button>

      <button on-click="handleSend">Send</button>
    `}}customElements.define("fivetwelve-manager",FivetwelveManager);class DekkManager extends reduxMixin(PolymerElement){static get properties(){return{live:{type:Boolean,statePath:"live"},editMode:{type:Boolean,computed:"computeEditMode(live)"},connected:{type:Boolean,statePath:"dekkManager.connected"},data:{type:Object,statePath:"dekkManager.data"},url:String,connectedLabel:{type:String,computed:"computeConnectedLabel(connected)"}}}computeEditMode(live){return!live}computeConnectedLabel(connected){return connected?"disconnect":"connect"}connectedCallback(){super.connectedCallback();this.url="ws://localhost:3001/luminave";if(this.connected){this.createWebsocket()}}handleClick(){if(this.connected){this.socket.close();this.dispatch(connectDekk(!1))}else{this.createWebsocket()}}createWebsocket(){this.socket=new WebSocket(this.url);this.socket.addEventListener("open",()=>{console.info("Dekk WebSocket opened to",this.url);this.dispatch(connectDekk(!0))});this.socket.addEventListener("close",event=>{console.info("Dekk WebSocket closed:",event);this.dispatch(connectDekk(!1))});this.socket.addEventListener("error",error=>{console.error("Dekk WebSocket error:",error);this.dispatch(connectDekk(!1))});this.socket.addEventListener("message",event=>{const{data}=event;this.changeScenes(this.data.scenes,"remove");this.dispatch(setDekkData({scenes:JSON.parse(data)}));this.changeScenes(this.data.scenes,"add")})}changeScenes(sceneNames,action){const arrayType=Array.isArray(sceneNames)?sceneNames:[sceneNames];arrayType.map(name=>{const scene=getSceneByName(this.getState(),{name});if(scene===void 0){console.log(`Scene "${name}" doesn't exist`)}else{switch(action){case"remove":this.dispatch(removeSceneFromTimelineAndResetFixtures(scene.id));break;case"add":this.dispatch(addSceneToTimeline(scene.id));break;default:}}})}static get template(){return html`
      Dekk: <button on-click="handleClick">[[connectedLabel]]</button>
    `}}customElements.define("dekk-manager",DekkManager);class LuminaveStatusIndicator extends PolymerElement{static get properties(){return{status:{type:Boolean,value:!1}}}computeClass(status){let statusClass="";if(status){statusClass="active"}else{statusClass="inactive"}return statusClass}static get template(){return html`
        <style>
          .active {
    ...
    `}}customElements.define("luminave-status-indicator",LuminaveStatusIndicator);class ColorGrid extends PolymerElement{static get properties(){return{rows:Number,colors:Array}}computeGridVars(rows){const vars={"--rows":rows};return Object.keys(vars).map(key=>[key,vars[key]].join(":")).join(";")}computeItemVars(color){const vars={"--red":color[0],"--green":color[1],"--blue":color[2]};return Object.keys(vars).map(key=>[key,vars[key]].join(":")).join(";")}_toColorArray(colors){const colorChunks=[];let i=0;while(i<colors.length){const chunk={color:colors.slice(i,i+3)};colorChunks.push(chunk);i+=3}return colorChunks}computeLabel(index){return`${index+1}`}static get template(){return html`
      <style>
        .container {
          display: grid;
    ...
    `}}customElements.define("color-grid",ColorGrid);class UiSpacer extends PolymerElement{static get properties(){return{height:{type:Number,value:"1em"}}}computeVars(height){const vars={"--height":height};return Object.keys(vars).map(key=>[key,vars[key]].join(":")).join(";")}static get template(){return html`
      <style>
        div {
          width: 100%;
          height: var(--height);
    ...
    `}}customElements.define("ui-spacer",UiSpacer);class LuminaveStatus extends reduxMixin(PolymerElement){static get properties(){return{bpm:{type:Number,statePath:"bpm"},live:{type:Boolean,statePath:"live"},editMode:{type:Boolean,computed:"computeEditMode(live)"},usbConnected:{type:Boolean,statePath:"connectionManager.usb.connected"},modvConnected:{type:Boolean,statePath:"modvManager.connected"},fivetwelveConnected:{type:Boolean,statePath:"fivetwelveManager.connected"}}}computeEditMode(live){return!live}static get template(){return html`
      <style>
        .grid {
          display: flex;
          flex-direction: row;
      ...
      <ui-spacer></ui-spacer>
      <ui-spacer></ui-spacer>
    `}}customElements.define("luminave-status",LuminaveStatus);class TapButton extends reduxMixin(PolymerElement){constructor(){super();this.bpm=0;this.arr=[];this.date={then:new Date,now:new Date};this.timer=setTimeout(()=>{},0);this.ticking=!1;this.average=0}ready(){super.ready();this.options={delay:this.attributes.delay||{value:2e3},items:this.attributes.items||{value:4}};this.delay=this.options.delay.value;this.items=this.options.items.value}handleClick(){this.run()}run(){clearTimeout(this.timer);this.timer=setTimeout(()=>{this.ticking=!1;this.arr=[]},this.delay);this.date.then=this.date.now;this.date.now=new Date;this.diff=this.date.now-this.date.then;if(this.ticking){this.arr.push(this.diff);if(this.arr.length>this.items){const diffs=this.arr.reduce((result,t)=>result+=t);this.arr.shift();this.average=diffs/this.arr.length;this.bpm=~~(6e4/this.average);this.dispatch(setBpm(this.bpm))}}else{this.ticking=!0}}static get template(){return html`
      <button on-click="handleClick">TAP</button>
    `}}customElements.define("tap-button",TapButton);var index$14={TapButton:TapButton};class BPMMeter extends PolymerElement{static get properties(){return{bpm:{type:Number,statePath:"bpm"}}}static get template(){return html`
      <div>[[bpm]]</div>
    `}}customElements.define("bpm-meter",BPMMeter);const actions={usb:connectUsb,bluetooth:connectBluetooth};class ConnectButton extends reduxMixin(PolymerElement){static get properties(){return{label:{type:String},type:{type:String},connections:{type:Object,statePath:"connectionManager",observer:"connectionChanged"}}}connectionChanged(){this.connected=this.connections[this.type].connected}computeVars(connected){const vars={"--on":connected?1:0,"--off":connected?0:1};return Object.keys(vars).map(key=>{return[key,vars[key]].join(":")}).join(";")}static get template(){return html`
        <style>
        button {
    ...
    `}}customElements.define("connect-button",ConnectButton);class StorageManager extends reduxMixin(PolymerElement){static get properties(){return{live:{type:Boolean,statePath:"live"},editMode:{type:Boolean,computed:"computeEditMode(live)"}}}computeEditMode(live){return!live}ready(){super.ready();store.subscribe(()=>{if(this.editMode){localStorage.setItem(STORAGE_STATE,JSON.stringify(store.getState()))}})}resetStorage(){localStorage.removeItem(STORAGE_STATE);window.location.reload(!0)}printStorage(){console.log(store.getState())}downloadStorage(){return`data:text/json;charset=utf-8,${encodeURIComponent(localStorage.getItem(STORAGE_STATE))}`}static get template(){return html`
      <!--<button on-click="resetStorage">Reset storage</button>
      <button on-click="printStorage">Print storage</button>-->
    `}}customElements.define("storage-manager",StorageManager);class LiveMode extends reduxMixin(PolymerElement){static get properties(){return{live:{type:Boolean,statePath:"live"},liveLabel:{type:String,computed:"computeLiveLabel(live)"}}}computeLiveLabel(){return this.live?"Live":"edit"}handleLive(){this.dispatch(setLive(!this.live))}static get template(){return html`
      <div>
        Mode: <button on-click="handleLive">[[liveLabel]]</button>
      </div>
    `}}customElements.define("live-mode",LiveMode);class ConfigManager extends reduxMixin(PolymerElement){static get properties(){return{live:{type:Boolean,statePath:"live"},editMode:{type:Boolean,computed:"computeEditMode(live)"}}}computeEditMode(live){return!live}generateDownload(){return`data:text/json;charset=utf-8,${encodeURIComponent(localStorage.getItem(STORAGE_STATE))}`}handleDownload(e){const{target}=e;console.log(target)}static get template(){return html`
      Config
      <a on-click="handleDownload" href="{{generateDownload()}}">Download</a>
    `}}customElements.define("config-manager",ConfigManager);class RainbowText extends PolymerElement{static get properties(){return{text:String,duration:{type:String,value:"12s"},rainbowChars:{type:Array,computed:"computeText(text)"}}}computeText(text){return text.split("")}computeVars(duration,rainbowChars,index){const vars={"--length":rainbowChars.length,"--delay":index,"--duration":duration};return Object.keys(vars).map(key=>[key,vars[key]].join(":")).join(";")}static get template(){return html`
      <style>
        :root {
          --duration: 1s;
          --length: 0;
    ...
    `}}customElements.define("rainbow-text",RainbowText);class LuminaveMenu extends reduxMixin(PolymerElement){static get properties(){return{bpm:{type:Number,statePath:"bpm"},live:{type:Boolean,statePath:"live"},editMode:{type:Boolean,computed:"computeEditMode(live)"}}}computeEditMode(live){return!live}toggleDrawer(){this.$.drawer.toggle()}static get template(){return html`
      <style>
        .grid {
          display: flex;
          flex-direction: column;
    ...
    `}}customElements.define("luminave-menu",LuminaveMenu);class ChannelGrid extends PolymerElement{static get properties(){return{channels:Array}}static get template(){return html`
      <style>
        .items {
          display: flex;
          flex-wrap: wrap;
    ...
    `}}customElements.define("channel-grid",ChannelGrid);class UniverseManager extends reduxMixin(PolymerElement){static get properties(){return{universes:{type:Array,statePath:"universeManager"},live:{type:Boolean,statePath:"live"},editMode:{type:Boolean,computed:"computeEditMode(live)"}}}computeEditMode(live){return!live}addUniverse(){const id=uuidV1();this.dispatch(addUniverse({id,channels:[...Array(512)].map(()=>0),name:`${id}`}))}removeUniverse(e){const{dataset}=e.target;this.dispatch(removeUniverse(parseInt(dataset.index,10)))}resetUniverse(e){const{dataset}=e.target;this.dispatch(resetUniverseAndFixtures(parseInt(dataset.index,10)))}static get template(){return html`
    ...
    `}}customElements.define("universe-manager",UniverseManager);class DmxFixtureProperty extends PolymerElement{static get properties(){return{name:{type:String},type:{type:String},channels:{type:Number},property:{type:Object}}}handleColorChange(e){const value=e.target.value.match(/[A-Za-z0-9]{2}/g).map(v=>parseInt(v,16));this.dispatchEvent(new CustomEvent("change",{detail:{value,name:this.name}}))}handleInputChange(e){let{value}=e.target;if(!isNaN(value)){value=parseInt(value,10)}this.dispatchEvent(new CustomEvent("change",{detail:{value,name:this.name}}))}handleSelectChange(e){const[selectedOption]=e.target.selectedOptions,{value}=selectedOption;this.dispatchEvent(new CustomEvent("change",{detail:{value,name:this.name}}))}static get template(){return html`
    <div>
    ...
    `}}customElements.define("dmx-fixture-property",DmxFixtureProperty);class DmxDevice$2 extends fivetwelve.DmxDevice{constructor(options){super(options);this.universe=options.universe}getChannelValue(channelNumber){if(!Number.isInteger(channelNumber)){throw new Error(`invalid channel-number: ${channelNumber}`,"INVALID_CHANNEL")}return batch[this.bufferOffset+channelNumber-1]}setChannelValue(channelNumber,value){if(!Number.isInteger(channelNumber)){throw new Error(`invalid channel-number: ${channelNumber}`,"INVALID_CHANNEL")}addToBatch(this.bufferOffset+channelNumber-1,Math.round(value))}getParamsList(){return Object.entries(this.params).filter(([name,param])=>param instanceof DmxParam).map(([name,param])=>{const{min=0,max=0,map={},channels}=param,mapping=param.mapping?Object.keys(param.mapping):["",...Object.keys(map)];return{min,max,name,channels,mapping,isRgb:param instanceof RgbParam,isRange:param instanceof RangeParam,isMapped:param instanceof MappedParam,isMultiRange:param instanceof MultiRangeParam,isHiRes:param instanceof HiResParam}})}}var DmxDevice$3={default:DmxDevice$2};class AdjStarburst extends DmxDevice$2{constructor(options){super(Object.assign({},options,{params:{color:new RgbParam([1,2,3]),white:new RangeParam(4,{min:0,max:255}),yellow:new RangeParam(5,{min:0,max:255}),uv:new RangeParam(6,{min:0,max:255}),strobeMacro:new MultiRangeParam(7,{off:{range:[0,7]},on:{range:[8,15],values:[8,15]},slowToFast:{range:[16,131],values:[16,131]},slowOpenToFastClose:{range:[140,181],values:[140,181]},fastOpenToSlowClose:{range:[190,231],values:[190,231]},random:{range:[240,247],values:[240,247]}}),dimmer:new RangeParam(8,{min:0,max:255}),rotate:new MultiRangeParam(9,{off:{range:[0,30]},clockwise:{range:[31,140],values:[0,255]},stop:{range:[141,145]},counterClockwise:{range:[146,255],values:[255,0]}})}}));this.layout={};this.layout.width=1;this.layout.height=1;this.channels=9;this.weight=3.65}}var AdjStarburst$1={default:AdjStarburst};class CameoFlatPar1RGBW extends DmxDevice$2{constructor(options){super(Object.assign({},options,{params:{color:new RgbParam([3,4,5]),white:new RangeParam(6,{min:0,max:255}),dimmer:new RangeParam(1,{min:0,max:255}),strobe:new RangeParam(2,{min:0,max:255}),colorMacro:new RangeParam(7,{min:0,max:255}),sound:new RangeParam(8,{min:0,max:255})}}));this.layout={};this.layout.width=1;this.layout.height=1;this.channels=8;this.weight=1.2}}var CameoFlatPar1RGBW$1={default:CameoFlatPar1RGBW};class CameoPixBar600PRO extends DmxDevice$2{constructor(options){super(Object.assign({},options,{params:{dimmer:new RangeParam(1,{min:0,max:255}),strobe:new RangeParam(2,{min:0,max:255}),led1:{color:new RgbParam([3,4,5]),white:new RangeParam(6,{min:0,max:255}),amber:new RangeParam(7,{min:0,max:255}),uv:new RangeParam(8,{min:0,max:255})},led2:{color:new RgbParam([9,10,11]),white:new RangeParam(12,{min:0,max:255}),amber:new RangeParam(13,{min:0,max:255}),uv:new RangeParam(14,{min:0,max:255})},led3:{color:new RgbParam([15,16,17]),white:new RangeParam(18,{min:0,max:255}),amber:new RangeParam(19,{min:0,max:255}),uv:new RangeParam(20,{min:0,max:255})},led4:{color:new RgbParam([21,22,23]),white:new RangeParam(24,{min:0,max:255}),amber:new RangeParam(25,{min:0,max:255}),uv:new RangeParam(26,{min:0,max:255})},led5:{color:new RgbParam([27,28,29]),white:new RangeParam(30,{min:0,max:255}),amber:new RangeParam(31,{min:0,max:255}),uv:new RangeParam(32,{min:0,max:255})},led6:{color:new RgbParam([33,34,35]),white:new RangeParam(36,{min:0,max:255}),amber:new RangeParam(37,{min:0,max:255}),uv:new RangeParam(38,{min:0,max:255})},led7:{color:new RgbParam([39,40,41]),white:new RangeParam(42,{min:0,max:255}),amber:new RangeParam(43,{min:0,max:255}),uv:new RangeParam(44,{min:0,max:255})},led8:{color:new RgbParam([45,46,47]),white:new RangeParam(48,{min:0,max:255}),amber:new RangeParam(49,{min:0,max:255}),uv:new RangeParam(50,{min:0,max:255})},led9:{color:new RgbParam([51,52,53]),white:new RangeParam(54,{min:0,max:255}),amber:new RangeParam(55,{min:0,max:255}),uv:new RangeParam(56,{min:0,max:255})},led10:{color:new RgbParam([57,58,59]),white:new RangeParam(60,{min:0,max:255}),amber:new RangeParam(61,{min:0,max:255}),uv:new RangeParam(62,{min:0,max:255})},led11:{color:new RgbParam([63,64,65]),white:new RangeParam(66,{min:0,max:255}),amber:new RangeParam(67,{min:0,max:255}),uv:new RangeParam(68,{min:0,max:255})},led12:{color:new RgbParam([69,70,71]),white:new RangeParam(72,{min:0,max:255}),amber:new RangeParam(73,{min:0,max:255}),uv:new RangeParam(74,{min:0,max:255})}}}));this.layout={};this.layout.width=12;this.layout.height=1;this.channels=74;this.weight=6}set color(value){this.led1.color=value;this.led2.color=value;this.led3.color=value;this.led4.color=value;this.led5.color=value;this.led6.color=value;this.led7.color=value;this.led8.color=value;this.led9.color=value;this.led10.color=value;this.led11.color=value;this.led12.color=value}set uv(value){this.led1.uv=value;this.led2.uv=value;this.led3.uv=value;this.led4.uv=value;this.led5.uv=value;this.led6.uv=value;this.led7.uv=value;this.led8.uv=value;this.led9.uv=value;this.led10.uv=value;this.led11.uv=value;this.led12.uv=value}}var CameoPixBar600PRO$1={default:CameoPixBar600PRO};class CameoWookie200RGY extends DmxDevice$2{constructor(options){super(Object.assign({},options,{params:{mode:new MappedParam(1,{off:[0,63],auto:[64,127],sound:[128,191],dmx:[192,255]}),colors:new MappedParam(2,{original:[0,63],red:[64,127],green:[128,191],yellow:[192,255]}),pattern:new MappedParam(3,{1:[0,7],2:[8,15],3:[16,23],4:[24,31],5:[32,39],6:[40,47],7:[48,55],8:[56,63],9:[64,71],10:[72,79],11:[80,87],12:[88,95],13:[96,103],14:[104,111],15:[112,119],16:[120,127],17:[128,135],18:[136,143],19:[144,151],20:[152,159],21:[160,167],22:[168,175],23:[176,183],24:[184,191],25:[192,199],26:[200,207],27:[208,215],28:[216,223],29:[224,231],30:[232,239],31:[240,247],32:[248,255]}),zoom:new MultiRangeParam(4,{manual:{range:[0,127],values:[0,255]},loop:{range:[128,255],values:[0,255]}}),xAxisRolling:new MultiRangeParam(5,{manual:{range:[0,127],values:[0,255]},speed:{range:[128,255],values:[0,255]}}),yAxisRolling:new MultiRangeParam(6,{manual:{range:[0,127],values:[0,255]},speed:{range:[128,255],values:[0,255]}}),zAxisRolling:new MultiRangeParam(7,{manual:{range:[0,127],values:[0,255]},speed:{range:[128,255],values:[0,255]}}),xAxisMoving:new MultiRangeParam(8,{manual:{range:[0,127],values:[0,255]},speed:{range:[128,255],values:[0,255]}}),yAxisMoving:new MultiRangeParam(9,{manual:{range:[0,127],values:[0,255]},speed:{range:[128,255],values:[0,255]}})}}));this.layout={};this.layout.width=1;this.layout.height=1;this.channels=9;this.weight=2}}var CameoWookie200RGY$1={default:CameoWookie200RGY};class EuroliteB100 extends DmxDevice$2{constructor(options){super(Object.assign({},options,{params:{motor:new MappedParam(1,{off:[0,127],on:[128,255]}),fan:new MappedParam(2,{off:[0,127],on:[128,255]})}}));this.layout={};this.layout.width=1;this.layout.height=1;this.channels=2;this.weight=2.3}}var EuroliteB100$1={default:EuroliteB100};class EuroliteTMH8 extends DmxDevice$2{constructor(options){super(Object.assign(options,{params:{pan:new HiResParam([1,2],{min:-270,max:270}),tilt:new HiResParam([3,4],{min:-115,max:115}),speed:new RangeParam(5,{min:0,max:255}),brightness:new MultiRangeParam(6,{dimmer:{range:[8,134],values:[0,255]},strobe:{range:[135,232],values:[0,255]}}),color:new RgbParam([7,8,9]),white:new RangeParam(10,{min:0,max:255}),colorSpeed:new RangeParam(12,{min:0,max:255}),movement:new MappedParam(13,EuroliteTMH8.MOVEMENTS),gobo:new MappedParam(14,EuroliteTMH8.GOBOS)}}));this.channels=14;this.weight=3}}EuroliteTMH8.MOVEMENTS={macro1:[8,23],macro2:[24,39],macro3:[40,55],macro4:[56,71],macro5:[72,87],macro6:[88,103],macro7:[104,119],macro8:[120,135]};EuroliteTMH8.GOBOS={clear:[0,7],flower:[8,15],spiral:[16,23],arrowCircle:[24,31],hearts:[32,39],flowers:[40,47],shards:[48,55],spiraltunnel:[56,63],bars:[64,71],flowers2:[72,79],flowerShake:[80,94],spiralShake:[95,109],arrowCircleShake:[110,124],heartsShake:[125,139],flowersShake:[140,154],shardsShake:[155,169],spiraltunnelShake:[170,184],barsShake:[185,199],flowers2Shake:[200,214]};var EuroliteTMH8$1={default:EuroliteTMH8};class FunGenerationSeParQuadLedRgbUv extends DmxDevice$2{constructor(options){super(Object.assign({},options,{params:{color:new RgbParam([1,2,3]),uv:new RangeParam(4,{min:0,max:255}),dimmer:new RangeParam(5,{min:0,max:255}),strobe:new RangeParam(6,{min:0,max:255})}}));this.channels=6;this.weight=1.1}}var FunGenerationSeParQuadLedRgbUv$1={default:FunGenerationSeParQuadLedRgbUv};class JsFestMovingHeadWash extends DmxDevice$2{constructor(options){super(Object.assign(options,{params:{pan:new HiResParam([1,2],{min:-315,max:315}),tilt:new HiResParam([3,4],{min:-140,max:140}),dimmer:new RangeParam(5,{min:0,max:255}),strobeMacro:new MultiRangeParam(6,{off:{range:[0,6]},slowToFast:{range:[7,65],values:[7,65]},pulseSlowToFast:{range:[70,128],values:[70,128]},fadingSlowToFast:{range:[133,191],values:[133,191]},fadingOutSlowToFast:{range:[196,255],values:[196,255]}}),color:new RgbParam([7,8,9]),white:new RangeParam(10,{min:0,max:255}),zoom:new RangeParam(11,{min:0,max:255}),scanSpeed:new RangeParam(12,{min:0,max:255}),rgbwMacro:new RangeParam(13,{min:0,max:255}),resetAuto:new RangeParam(14,{min:0,max:255})}}));this.channels=14;this.weight=5}}JsFestMovingHeadWash.MOVEMENTS={macro1:[8,23],macro2:[24,39],macro3:[40,55],macro4:[56,71],macro5:[72,87],macro6:[88,103],macro7:[104,119],macro8:[120,135]};var JsFestMovingHeadWash$1={default:JsFestMovingHeadWash};class StairvilleAF150 extends DmxDevice$2{constructor(options){super(Object.assign({},options,{params:{amount:new RangeParam(1,{min:0,max:255})}}));this.layout={};this.layout.width=1;this.layout.height=1;this.channels=1;this.weight=5.5}}var StairvilleAF150$1={default:StairvilleAF150};class StairvilleBowlBeam604LEDCOBMovingHead extends DmxDevice$2{constructor(options){super(Object.assign(options,{params:{pan:new HiResParam([1,2],{min:-270,max:270}),tilt:new HiResParam([3,4],{min:-135,max:135}),panTilt:new RangeParam(5,{min:0,max:255}),panEndless:new RangeParam(6,{min:0,max:255}),tiltEndless:new RangeParam(7,{min:0,max:255}),color:new RgbParam([8,9,10]),white:new RangeParam(11,{min:0,max:255}),strobe:new RangeParam(12,{min:0,max:255}),dimmer:new RangeParam(13,{min:0,max:255})}}));this.channels=33;this.weight=5.1}}var StairvilleBowlBeam604LEDCOBMovingHead$1={default:StairvilleBowlBeam604LEDCOBMovingHead};class StairvilleOutdoorStageParTri extends DmxDevice$2{constructor(options){super(Object.assign({},options,{params:{color:new RgbParam([2,3,4]),dimmer:new RangeParam(1,{min:0,max:255}),strobe:new RangeParam(5,{min:0,max:255})}}));this.layout={};this.layout.width=1;this.layout.height=1;this.channels=8;this.weight=3.5}}var StairvilleOutdoorStageParTri$1={default:StairvilleOutdoorStageParTri};class BasicColor extends DmxDevice$2{constructor(options){super(Object.assign({},options,{params:{color:new RgbParam([1,2,3])}}));this.layout={};this.layout.width=1;this.layout.height=1;this.channels=3;this.weight=0}}var BasicColor$1={default:BasicColor};class BasicFluter extends DmxDevice$2{constructor(options){super(Object.assign({},options,{params:{color:new RgbParam([4,5,6]),white:new RangeParam(7,{min:0,max:255}),dimmer:new RangeParam(3,{min:0,max:255}),strobe:new RangeParam(1,{min:0,max:255})}}));this.layout={};this.layout.width=1;this.layout.height=1;this.channels=7;this.weight=1.2}}var BasicFluter$1={default:BasicFluter},Fixtures={AdjStarburst:AdjStarburst,CameoFlatPar1RGBW:CameoFlatPar1RGBW,CameoPixBar600PRO:CameoPixBar600PRO,CameoWookie200RGY:CameoWookie200RGY,EuroliteB100:EuroliteB100,EuroliteTMH8:EuroliteTMH8,FunGenerationSeParQuadLedRgbUv:FunGenerationSeParQuadLedRgbUv,JsFestMovingHeadWash:JsFestMovingHeadWash,StairvilleAF150:StairvilleAF150,StairvilleBowlBeam604LEDCOBMovingHead:StairvilleBowlBeam604LEDCOBMovingHead,StairvilleOutdoorStageParTri:StairvilleOutdoorStageParTri,BasicColor:BasicColor,BasicFluter:BasicFluter};class DmxFixture extends reduxMixin(PolymerElement){static get properties(){return{name:{type:String},id:{type:String},type:{type:String},address:{type:Number},universe:{type:Number},fixture:{type:Object},properties:Object,_properties:{type:Object,computed:"computeProperties(fixture)"}}}ready(){super.ready();this.fixture=new Fixtures[this.type]({address:this.address,universe:this.universe})}handleSubmit(e){e.preventDefault();const data=new FormData(e.target),address=parseInt(data.get("address"),10);this.dispatch(setFixtureAddress(this.id,address))}computeProperties(fixture){return fixture.getParamsList()}handleChange(e){const{name,value}=e.detail;this.fixture[name]=value;this.dispatch(setChannels(0,[...batch]));const now=new Date;window.dispatchEvent(new CustomEvent("send-universe-to-usb-dmx-controller",{detail:{now}}));window.dispatchEvent(new CustomEvent("send-universe-to-fivetwelve",{detail:{now}}))}static get template(){return html`
        <style>
          .grid {
            display: flex;
            flex-direction: row;
    ...
    `}}customElements.define("dmx-fixture",DmxFixture);class FixtureManager extends reduxMixin(PolymerElement){constructor(){super();this.types=FIXTURE_TYPES;this.types.sort()}static get properties(){return{fixtures:{type:Array,statePath:"fixtureManager"}}}removeFixture(e){const{dataset}=e.target;this.dispatch(removeFixtureFromEverywhere(dataset.id))}handleSubmit(e){e.preventDefault();const data=new FormData(e.target),type=data.get("type"),name=data.get("name"),universe=0,address=parseInt(data.get("address"),10),amount=parseInt(data.get("amount"),10);if(isNaN(amount)){this.dispatch(addFixture({id:uuidV1(),type,name,universe,address,properties:{}}))}else{const bulkFixture=new Fixtures[type]({address,universe}),offset=bulkFixture.channels;for(let i=0;i<amount;i++){const bulkAddress=address+offset*i;this.dispatch(addFixture({id:uuidV1(),type,name:`${name}${bulkAddress}`,universe,address:bulkAddress,properties:{}}))}}}static get template(){return html`
    <style>
      :host {
        --width: 4;
      }
    ...
    `}}customElements.define("fixture-manager",FixtureManager);class FixtureListItem extends PolymerElement{static get properties(){return{fixture:Object}}static get template(){return html`
      <div>
        [[fixture.name]]
      </div>
    `}}customElements.define("fixture-list-item",FixtureListItem);class FixtureList extends PolymerElement{static get properties(){return{fixtures:Array,fixtureManager:Array}}handleFixtureSubmit(e){const{path}=e,[form]=path,[select]=form,fixtureIds=[...select.selectedOptions].map(fixture=>fixture.value);this.dispatchEvent(new CustomEvent("add-fixtures",{detail:{event:e,fixtureIds}}))}handleSelectedFixtures(e){this.fixtureIds=e.target.selectedOptions}handleRemoveFixture(e){this.dispatchEvent(new CustomEvent("remove-fixture",{detail:{event:e,fixtureIndex:e.target.fixtureIndex}}))}getFixture(fixtureId){return this.fixtureManager.filter(fixture=>fixture.id===fixtureId)[0]}static get template(){return html`
      <style>
        .items {
          display: flex;
          flex-wrap: wrap;
    ...
    `}}customElements.define("fixture-list",FixtureList);class AnimationListItem extends PolymerElement{static get properties(){return{animation:Object}}static get template(){return html`
      <div>
        [[animation.name]]
      </div>
    `}}customElements.define("animation-list-item",AnimationListItem);class AnimationList extends PolymerElement{static get properties(){return{animations:Array,animationManager:Array}}handleAnimationSubmit(e){this.dispatchEvent(new CustomEvent("add-animation",{detail:{event:e,animationId:this.animationId}}))}handleSelectedAnimation(e){this.animationId=e.target.selectedOptions[0].value}handleRemoveAnimation(e){this.dispatchEvent(new CustomEvent("remove-animation",{detail:{event:e,animationIndex:e.target.animationIndex}}))}getAnimation(animationId){return this.animationManager.filter(animation=>animation.id===animationId)[0]}static get template(){return html`
      <style>
        .items {
          display: flex;
          flex-wrap: wrap;
    ...
    `}}customElements.define("animation-list",AnimationList);class SceneBee extends reduxMixin(PolymerElement){static get properties(){return{name:String,duration:Number,id:String,index:Number,fixtures:Array,fixtureManager:{type:Array,statePath:"fixtureManager"},animations:Array,animationManager:{type:Array,statePath:"animationManager"}}}runScene(){this.dispatch(addSceneToTimeline(this.id))}removeScene(e){const{dataset}=e.target;this.dispatch(removeScene(parseInt(dataset.index,10)))}handleAddAnimation(e){const{event,animationId}=e.detail;event.preventDefault();event.target.reset();this.dispatch(addAnimationToScene(this.index,animationId))}handleRemoveAnimation(e){const{animationIndex}=e.detail;this.dispatch(removeAnimationFromScene(this.index,animationIndex));this.dispatch(resetUniverseAndFixtures(0))}handleAddFixtures(e){const{event,fixtureIds}=e.detail;event.preventDefault();this.dispatch(addFixturesToScene(this.index,fixtureIds))}handleRemoveFixture(e){const{fixtureIndex}=e.detail;this.dispatch(removeFixtureFromScene(this.id,fixtureIndex))}handleNameChange(e){const sceneName=e.target.value;this.dispatch(setSceneName(this.index,sceneName))}static get template(){return html`
    <style>
      h4 {
        margin: 0.25em 0;
      }
    ...
    `}}customElements.define("scene-bee",SceneBee);class SceneManager extends reduxMixin(PolymerElement){static get properties(){return{scenes:{type:Array,statePath:getScenesSorted},fixtureManager:{type:Array,statePath:"fixtureManager"},animationManager:{type:Array,statePath:"animationManager"}}}handleSubmitScene(e){e.preventDefault();this.dispatch(addScene({id:uuidV1(),fixtures:[],animations:[],duration:this.duration,name:this.name,isRunning:!1}))}handleName(e){this.name=e.target.value}handleDuration(e){this.duration=e.target.value}handleSubmitSceneAnimationFixtures(e){e.preventDefault();const data=new FormData(e.target),name=data.get("name");this.dispatch(addScene({id:uuidV1(),fixtures:this.fixtures,animations:this.animations,duration:20,name,isRunning:!1}));this.fixtures=[];this.animations=[]}handleAddAnimation(e){const{event,animationId}=e.detail;event.preventDefault();this.animations=[animationId]}handleAddFixtures(e){const{event,fixtureIds}=e.detail;event.preventDefault();this.fixtures=fixtureIds}static get template(){return html`
      <style>
        :host {
          --width: 4;
        }
    ...
    `}}customElements.define("scene-manager",SceneManager);class KeyframeGrid extends PolymerElement{static get properties(){return{keyframes:{type:Object}}}_toArray(object){const array=[];for(const step in object){array.push({step:parseFloat(step),value:object[step]})}array.sort((a,b)=>a.step-b.step);return array}_toJson(object){return JSON.stringify(object)}static get template(){return html`
      <style>
        .items {
          display: flex;
          flex-wrap: wrap;
    ...
    `}}customElements.define("keyframe-grid",KeyframeGrid);class AnimationBee extends reduxMixin(PolymerElement){constructor(){super();this.properties=FIXTURE_PROPERTIES;this.properties.sort()}static get properties(){return{name:String,duration:Number,index:Number,keyframes:Object}}removeAnimation(){this.dispatch(removeAnimation(parseInt(this.index,10)))}handleKeyframeSubmit(e){e.preventDefault();this.dispatch(addKeyframe(this.index,this.step,this.property,this.value))}handleStep(e){this.step=e.target.value}handleSelectedProperty(e){this.property=e.target.selectedOptions[0].value}handleValue(e){this.value=e.target.value}handleNameChange(e){const animationName=e.target.value;this.dispatch(setAnimationName(this.index,animationName))}static get template(){return html`
      <style>
        .name {
          display: inline-block;
          width: 100%;
    ...
    `}}customElements.define("animation-bee",AnimationBee);class AnimationManager extends reduxMixin(PolymerElement){static get properties(){return{animations:{type:Array,statePath:getAnimationsSorted}}}handleSubmit(e){e.preventDefault();const data=new FormData(e.target),duration=parseInt(data.get("duration"),10),name=data.get("name"),amount=parseInt(data.get("amount"),10);if(isNaN(amount)){this.dispatch(addAnimation({id:uuidV1(),keyframes:{},duration,name}))}else{for(let i=0;i<amount;i++){const animationIndex=i+1;this.dispatch(addAnimation({id:uuidV1(),keyframes:{0:{modvColor:animationIndex},1:{modvColor:animationIndex}},duration,name:`${name}${animationIndex}`}))}}}static get template(){return html`
    <style>
      .container {
        --width: 8;
        display: grid;
    ...
    `}}customElements.define("animation-manager",AnimationManager);class SceneListItem extends PolymerElement{static get properties(){return{scene:Object}}static get template(){return html`
      <div>
        [[scene.name]]
      </div>
    `}}customElements.define("scene-list-item",SceneListItem);class SceneList extends reduxMixin(PolymerElement){static get properties(){return{scenes:Array,sceneManager:Array,live:{type:Boolean,statePath:"live"},editMode:{type:Boolean,computed:"computeEditMode(live)"}}}computeEditMode(live){return!live}handleSceneSubmit(e){e.preventDefault();const data=new FormData(e.target),sceneIds=data.getAll("scenes");this.dispatchEvent(new CustomEvent("add-scenes",{detail:{event:e,sceneIds}}))}handleRemoveScene(e){this.dispatchEvent(new CustomEvent("remove-scene",{detail:{event:e,sceneIndex:e.target.sceneIndex}}))}getScene(sceneId){return this.sceneManager.filter(scene=>scene.id===sceneId)[0]}static get template(){return html`
      <template is="dom-if" if="[[editMode]]">
    ...
    `}}customElements.define("scene-list",SceneList);class MidiGrid extends reduxMixin(PolymerElement){constructor(){super();this.types=MIDI_TYPES;this.types.sort()}static get properties(){return{width:Number,height:Number,mapping:Array,controllerindex:Number,learnIndex:{type:Number,statePath:"midiManager.learning"},sceneManager:{type:Array,statePath:"sceneManager"},live:{type:Boolean,statePath:"live"},editMode:{type:Boolean,computed:"computeEditMode(live)"}}}computeEditMode(live){return!live}computeGridVars(width){const vars={"--width":width};return Object.keys(vars).map(key=>[key,vars[key]].join(":")).join(";")}computeItemVars(element,index,learnIndex){const vars={"--isLearning":index===learnIndex?0:1,"--isActive":element.active?0:1};return Object.keys(vars).map(key=>[key,vars[key]].join(":")).join(";")}handleLearn(e){const{dataset}=e.target,mappingIndex=parseInt(dataset.index,10);this.dispatch(learnMidi(mappingIndex))}handleAddScenes(e){const{event,sceneIds}=e.detail,{dataset}=e.target;event.preventDefault();event.target.reset();this.dispatch(addScenesToMidi(this.controllerindex,parseInt(dataset.index,10),sceneIds))}handleRemoveScene(e){const{sceneIndex}=e.detail,{dataset}=e.target,mappingIndex=parseInt(dataset.index,10);this.dispatch(removeSceneFromMidi(this.controllerindex,mappingIndex,sceneIndex))}handleLabelChange(e){const label=e.target.value,{dataset}=e.target,mappingIndex=parseInt(dataset.index,10);this.dispatch(addMidiMapping(this.controllerindex,mappingIndex,{label}))}handleType(e){e.preventDefault();const data=new FormData(e.target),type=data.get("type"),mappingIndex=parseInt(data.get("mappingIndex"),10);this.dispatch(addMidiMapping(this.controllerindex,mappingIndex,{type}))}selectedType(elementType,type){return elementType===type?"selected":""}showValueForType(type){if(type===MIDI_TYPE_KNOB||type===MIDI_TYPE_FADER){return!0}else{return!1}}isNotEmpty(type){return type!==MIDI_TYPE_EMPTY}static get template(){return html`
      <style>
        .container {
          display: grid;
          grid-template-columns: repeat(var(--width), auto);
...
-->
    ...
    `}}customElements.define("midi-grid",MidiGrid);class MidiController extends reduxMixin(PolymerElement){constructor(){super();this.connected=!1;this.input=null;this.output=null}ready(){super.ready();if(0===this.mapping.length){const elements=this.width*this.height;for(let i=0;i<elements;i++){this.dispatch(addMidiMapping(this.index,i,{scenes:[],note:-1,label:"",type:"",active:!1,value:0}))}}}static get properties(){return{name:String,id:String,index:Number,inputname:String,outputname:String,width:Number,height:Number,connected:Boolean,mapping:Array,midiLearning:{type:Number,statePath:"midiManager.learning"},midiEnabled:{type:Boolean,statePath:"midiManager.enabled",observer:"midiEnabledChanged"},live:{type:Boolean,statePath:"live"},editMode:{type:Boolean,computed:"computeEditMode(live)"}}}computeEditMode(live){return!live}midiEnabledChanged(){if(this.midiEnabled){WebMidi.addListener("connected",e=>{const{port}=e,{name,type}=port;if(name===this.inputname&&"input"===type&&null===this.input){this.input=port;this.input.addListener("noteon","all",this.noteon.bind(this));this.input.addListener("controlchange","all",this.controlchange.bind(this))}else if(name===this.outputname&&"output"===type){this.output=port}this.connected=!0});WebMidi.addListener("disconnected",e=>{const{name,type}=e.port;if(name===this.inputname&&"input"===type){this.input=null}else if(name===this.outputname&&"output"===type){this.output=null}this.connected=!1})}}noteon(event){const{data}=event,[channel,note,velocity]=data;if(-1<this.midiLearning){this.dispatch(addMidiMapping(this.index,this.midiLearning,{note}));this.dispatch(learnMidi(-1))}else{const mappingIndex=this.mapping.findIndex(element=>element.note===note);if(-1<mappingIndex){const element=this.mapping[mappingIndex];element.active=!element.active;this.dispatch(setMidiMappingActive(this.index,mappingIndex,element.active));if(element.active){element.scenes.map(sceneId=>this.dispatch(addSceneToTimeline(sceneId)));this.output.send(144,[note,127])}else{element.scenes.map(sceneId=>this.dispatch(removeSceneFromTimelineAndResetFixtures(sceneId)));this.output.send(144,[note,0])}}}}controlchange(event){const{data}=event,[,note,velocity]=data;if(-1<this.midiLearning){this.dispatch(addMidiMapping(this.index,this.midiLearning,{note}));this.dispatch(learnMidi(-1))}else{const mappingIndex=this.mapping.findIndex(element=>element.note===note);if(-1<mappingIndex){}}}static get template(){return html`
      <style>
        h3 {
          margin-top: 0;
        }
    ...
    `}}customElements.define("midi-controller",MidiController);class MidiManager extends reduxMixin(PolymerElement){constructor(){super();this.dispatch(enableMidi(!1));WebMidi.enable(err=>{if(err){console.error("Web MIDI API could not be enabled:",err)}else{WebMidi.addListener("connected",e=>{const{manufacturer,name,id,type}=e.port;console.log("MIDIController added:","Manufacturer:",manufacturer,"| Name:",name,"| ID:",id,"| Type:",type)});WebMidi.addListener("disconnected",e=>{const{manufacturer,name,type,id}=e.port;console.log("MIDIController removed:","Manufacturer:",manufacturer,"| Name:",name,"| ID:",id,"| Type:",type)});this.dispatch(enableMidi(!0))}})}static get properties(){return{controllers:{type:Array,statePath:"midiManager.controllers"},live:{type:Boolean,statePath:"live"},editMode:{type:Boolean,computed:"computeEditMode(live)"}}}computeEditMode(live){return!live}removeMidi(e){const{dataset}=e.target;this.dispatch(removeMidi(parseInt(dataset.index,10)))}handleController(e){console.log("handleController",e)}handleName(e){this.name=e.target.value}handleInput(e){this.input=e.target.value}handleOutput(e){this.output=e.target.value}handleWidth(e){this.width=e.target.value}handleHeight(e){this.height=e.target.value}handleSubmit(e){e.preventDefault();e.target.reset();this.dispatch(addMidi({id:uuidV1(),name:this.name,input:this.input,output:this.output,width:this.width,height:this.height,mapping:[]}))}static get template(){return html`
    <style>
      .grid {
        display: flex;
        flex-direction: column;
    ...
    `}}customElements.define("midi-manager",MidiManager);class TimelineAnimation extends PolymerElement{static get properties(){return{duration:Number,progress:{type:Number,observer:"observeProgress"},fixtureIds:Array,animation:Object,timeline:{type:Object,computed:"computeTimeline(animation.keyframes)"}}}computeTimeline(keyframes){keyframes={...keyframes};const keyframesArray=this._toArray(keyframes);let properties=[];for(const step in keyframes){properties=properties.concat(Object.keys(keyframes[step]))}properties=[...new Set(properties)];const timeline=properties.map(name=>{const property={name,keyframes:[],keyframes:keyframesArray.filter(keyframe=>keyframe.value[name]!==void 0).map(keyframe=>{return{time:keyframe.time,value:keyframe.value[name]}})};return property});return new keytime(timeline)}_toArray(object){const array=[];for(const step in object){array.push({time:parseFloat(step),value:object[step]})}return array.sort((a,b)=>a.time-b.time)}observeProgress(){const interpolatedProperties=this.timeline.values(this.computeProgress());for(let i=0;i<this.fixtureIds.length;i++){addToFixtureBatch(this.fixtureIds[i],interpolatedProperties)}}computeProgress(){let progress=this.progress/this.duration;if(1<progress){progress=1}return progress}static get template(){return html``}}customElements.define("timeline-animation",TimelineAnimation);class TimelineScene extends reduxMixin(PolymerElement){static get properties(){return{scene:Object,progress:Number}}_getAnimation(animationId){return getAnimation(this.getState(),{animationId})}static get template(){return html`
      <style>
        h3 {
          font-size: 1em;
          font-weight: normal;
    ...
    `}}customElements.define("timeline-scene",TimelineScene);class LuminaveButton extends PolymerElement{static get template(){return html`
        <style>
    ...
    `}}customElements.define("luminave-button",LuminaveButton);class TimelineManager extends reduxMixin(PolymerElement){constructor(){super();this.measures=20;this.timeoutId=void 0;document.addEventListener("keypress",e=>{const{code}=e;if("Space"===code&&e.target===document.body){e.preventDefault();this.handlePlay()}})}static get properties(){return{bpm:{type:Number,statePath:"bpm"},timelineScenes:{type:Array,statePath:getTimelineScenes},isPlaying:{type:Boolean,statePath:"timelineManager.playing",observer:"observePlaying"},playLabel:{type:String,computed:"computePlayLabel(isPlaying)"},allFixtures:{type:Array,statePath:getFixtures},timelineFixtures:{type:Object,computed:"computeFixtures(allFixtures)"},modvConnected:{type:Boolean,statePath:"modvManager.connected"}}}computePlayLabel(isPlaying){return isPlaying?"Pause":"Play"}handlePlay(){this.dispatch(playTimeline(!this.isPlaying))}handleReset(){this.dispatch(resetTimeline());this.dispatch(resetUniverseAndFixtures(0))}observePlaying(){if(this.isPlaying){console.log("playing");this.time=new Date;this.loop()}else{console.log("stopped");clearTimeout(this.timeoutId)}}normalizeProgress(progress){return progress.toFixed(2)}loop(){if(this.isPlaying){this.duration=~~(1e3*(60/this.bpm)*this.measures);const now=new Date,loopEnd=now-this.time>this.duration,timeCounter=(now-this.time)/this.duration;if(loopEnd){this.time=now}this.progress=this.normalizeProgress(timeCounter*this.measures);for(const fixtureId in fixtureBatch){const interpolatedProperties=fixtureBatch[fixtureId].properties,fixture=this.timelineFixtures[fixtureId];for(const propertyName in interpolatedProperties){fixture[propertyName]=interpolatedProperties[propertyName]}if(this.modvConnected&&fixture.hasOwnProperty("color")){if(fixture.hasOwnProperty("modvColor")){fixture.modvColor=Math.round(fixture.modvColor);const color=modvData.colors.slice(3*(fixture.modvColor-1),3*(fixture.modvColor-1)+3);fixture.color=3===color.length?color:fixture.color}else{fixture.color=modvData.average}}}this.dispatch(setChannels(0,[...batch]));clearBatch();window.dispatchEvent(new CustomEvent("send-universe-to-usb-dmx-controller",{detail:{now}}));window.dispatchEvent(new CustomEvent("send-universe-to-fivetwelve",{detail:{now}}));this.timeoutId=setTimeout(()=>{requestAnimationFrame(this.loop.bind(this))},1e3/30)}}computeFixtures(allFixtures){const fixtures={};allFixtures.forEach(fixture=>{fixtures[fixture.id]=new Fixtures[fixture.type]({address:fixture.address,universe:fixture.universe})});return fixtures}static get template(){return html`
      <style>
        .grid {
          display: flex;
          flex-direction: row;
    ...
    `}}customElements.define("timeline-manager",TimelineManager);class LuminaveDashboard extends reduxMixin(PolymerElement){static get properties(){return{bpm:{type:Number,statePath:"bpm"},live:{type:Boolean,statePath:"live"},editMode:{type:Boolean,computed:"computeEditMode(live)"},modvColors:Array}}computeEditMode(live){return!live}listenReceivedModvData(){this.modvColors=modvData.colors}connectedCallback(){super.connectedCallback();window.addEventListener("received-data-from-modv",this.listenReceivedModvData.bind(this))}disconnectedCallback(){super.disconnectedCallback();window.removeEventListener("received-data-from-modv",this.listenReceivedModvData.bind(this))}static get template(){return html`
      <style>
          paper-tabs {
            display: inline-block;
    ...
    `}}customElements.define("luminave-dashboard",LuminaveDashboard);class Luminave extends PolymerElement{constructor(){super()}static get template(){return html`
      <luminave-menu></luminave-menu>
      <luminave-dashboard></luminave-dashboard>
    `}}customElements.define("lumi-nave",Luminave);export{index as $index,uuid$1 as $uuid,index$4 as $index$1,DeviceGroup$1 as $DeviceGroup,DmxDevice$1 as $DmxDevice,index$3 as $index$2,fivetwelve as $index$3,DmxParam$1 as $DmxParam,HiResParam$1 as $HiResParam,MappedParam$1 as $MappedParam,MultiRangeParam$1 as $MultiRangeParam,RangeParam$1 as $RangeParam,RgbParam$1 as $RgbParam,index$2 as $index$4,DmxOutput$1 as $DmxOutput,index$1 as $index$5,Color$1 as $Color,clamp$1 as $clamp,rangeMapper$1 as $rangeMapper,index$5 as $index$6,index$6 as $index$7,appLayoutBehavior as $appLayoutBehavior,appScrollEffectsBehavior as $appScrollEffectsBehavior,helpers as $helpers,ironA11yKeysBehavior as $ironA11yKeysBehavior,ironButtonState as $ironButtonState,ironControlState as $ironControlState,ironMenuBehavior as $ironMenuBehavior,ironMenubarBehavior as $ironMenubarBehavior,ironMeta as $ironMeta,ironResizableBehavior as $ironResizableBehavior,ironScrollTargetBehavior as $ironScrollTargetBehavior,ironMultiSelectable as $ironMultiSelectable,ironSelectable as $ironSelectable,ironSelection as $ironSelection,paperButtonBehavior as $paperButtonBehavior,paperInkyFocusBehavior as $paperInkyFocusBehavior,paperRippleBehavior as $paperRippleBehavior,arraySelector as $arraySelector,customStyle as $customStyle,domBind as $domBind,domIf as $domIf,domModule as $domModule,domRepeat as $domRepeat,_class as $class,legacyElementMixin as $legacyElementMixin,mutableDataBehavior as $mutableDataBehavior,polymerFn as $polymerFn,polymer_dom as $polymerDom,templatizerBehavior as $templatizerBehavior,dirMixin as $dirMixin,elementMixin as $elementMixin,gestureEventListeners as $gestureEventListeners,mutableData as $mutableData,propertiesChanged as $propertiesChanged,propertiesMixin as $propertiesMixin,propertyAccessors as $propertyAccessors,propertyEffects as $propertyEffects,templateStamp as $templateStamp,arraySplice as $arraySplice,async as $async,caseMap$1 as $caseMap,debounce as $debounce,flattenedNodesObserver as $flattenedNodesObserver,flush$2 as $flush,gestures$0 as $gestures,htmlTag as $htmlTag,mixin as $mixin,path as $path,renderStatus as $renderStatus,resolveUrl$1 as $resolveUrl,settings as $settings,styleGather as $styleGather,templatize$1 as $templatize,polymerElement as $polymerElement,polymerLegacy as $polymerLegacy,applyShimUtils as $applyShimUtils,applyShim as $applyShim$1,commonRegex as $commonRegex,commonUtils as $commonUtils,cssParse as $cssParse,customStyleInterface as $customStyleInterface$1,documentWait$1 as $documentWait,styleSettings as $styleSettings,styleUtil as $styleUtil,templateMap$1 as $templateMap,unscopedStyleHandler as $unscopedStyleHandler,index$7 as $index$8,invariant$1 as $invariant,polymerRedux as $polymerRedux,index$8 as $index$9,index$9 as $index$10,controller as $controller,index$13 as $index$11,index$14 as $index$43,index$10 as $index$50,reducers as $index$51,reduxStore as $reduxStore,index$12 as $index$52,Fixtures as $dmxFixtures,AdjStarburst$1 as $AdjStarburst,BasicColor$1 as $BasicColor,BasicFluter$1 as $BasicFluter,CameoFlatPar1RGBW$1 as $CameoFlatPar1RGBW,CameoPixBar600PRO$1 as $CameoPixBar600PRO,CameoWookie200RGY$1 as $CameoWookie200RGY,DmxDevice$3 as $DmxDevice$1,EuroliteB100$1 as $EuroliteB100,EuroliteTMH8$1 as $EuroliteTMH8,FunGenerationSeParQuadLedRgbUv$1 as $FunGenerationSeParQuadLedRgbUv,JsFestMovingHeadWash$1 as $JsFestMovingHeadWash,StairvilleAF150$1 as $StairvilleAF150,StairvilleBowlBeam604LEDCOBMovingHead$1 as $StairvilleBowlBeam604LEDCOBMovingHead,StairvilleOutdoorStageParTri$1 as $StairvilleOutdoorStageParTri,index$11 as $index$53,abcQ as $indexDefault,generator as $uuidDefault,uuid,uuidV1,fivetwelve as $indexDefault$1,DeviceGroup as $DeviceGroupDefault,DmxDevice as $DmxDeviceDefault,DmxDevice,DeviceGroup,index$2 as param,DeviceGroup as DeviceGroup$1,DmxDevice as DmxDevice$1,DmxOutput,init as $indexDefault$2,DmxParam as $DmxParamDefault,HiResParam as $HiResParamDefault,MappedParam as $MappedParamDefault,MultiRangeParam as $MultiRangeParamDefault,RangeParam as $RangeParamDefault,RgbParam as $RgbParamDefault,RangeParam,DmxParam,HiResParam,MappedParam,RgbParam,MultiRangeParam,DmxOutput as $DmxOutputDefault,DmxOutput as DmxOutput$1,Color as $ColorDefault,clamp,rangeMapper,clampedRangeMapper,redux as $indexDefault$3,createStore,combineReducers,applyMiddleware,WebMidi as $indexDefault$4,AppLayoutBehavior,AppScrollEffectsBehavior,_scrollEffects,_scrollTimer,scrollTimingFunction,registerEffect,queryAllRoot,scroll,IronA11yKeysBehavior,IronButtonStateImpl,IronButtonState,IronControlState,IronMenuBehaviorImpl,IronMenuBehavior,IronMenubarBehaviorImpl,IronMenubarBehavior,IronMeta,IronResizableBehavior,IronScrollTargetBehavior,IronMultiSelectableBehaviorImpl,IronMultiSelectableBehavior,IronSelectableBehavior,IronSelection,PaperButtonBehaviorImpl,PaperButtonBehavior,PaperInkyFocusBehaviorImpl,PaperInkyFocusBehavior,PaperRippleBehavior,ArraySelectorMixin,ArraySelector,CustomStyle,DomBind,DomIf,DomModule,DomRepeat,mixinBehaviors,Class,LegacyElementMixin,MutableDataBehavior,OptionalMutableDataBehavior,Polymer,matchesSelector,DomApi,dom,flush$1 as flush,enqueueDebouncer as addDebouncer,Templatizer,DirMixin,ElementMixin,instanceCount,registrations,register,dumpRegistrations,updateStyles,GestureEventListeners,MutableData,OptionalMutableData,PropertiesChanged,PropertiesMixin,PropertyAccessors,PropertyEffects,TemplateStamp,calculateSplices,timeOut,animationFrame,idlePeriod,microTask,dashToCamelCase,camelToDashCase,Debouncer,FlattenedNodesObserver,enqueueDebouncer,flush$1,gestures,recognizers,deepTargetFind,addListener,removeListener,register$1,setTouchAction,prevent,resetMouseCanceller,findOriginalTarget,add,remove,html,htmlLiteral,dedupingMixin,isPath,root,isAncestor,isDescendant,translate,matches,normalize,split,get,set,isDeep,beforeNextRender,afterNextRender,flush as flush$2,resolveUrl,resolveCss,pathFromUrl,useShadow,useNativeCSSProperties,useNativeCustomElements,rootPath,setRootPath,sanitizeDOMValue,setSanitizeDOMValue,passiveTouchGestures,setPassiveTouchGestures,stylesFromModules,stylesFromModule,stylesFromTemplate,stylesFromModuleImports,cssFromModules,cssFromModule,cssFromTemplate,cssFromModuleImports,templatize,modelForElement,TemplateInstanceBase,html as html$1,PolymerElement,Polymer as Polymer$1,html as html$2,Base,invalidate,invalidateTemplate,isValid,templateIsValid,isValidating,templateIsValidating,startValidating,startValidatingTemplate,elementsAreInvalid,ApplyShim as $applyShimDefault,VAR_ASSIGN,MIXIN_MATCH,VAR_CONSUMED,ANIMATION_MATCH,MEDIA_MATCH,IS_VAR,BRACKETED,HOST_PREFIX,HOST_SUFFIX,updateNativeProperties,getComputedStyleValue,detectMixin,StyleNode,parse,stringify,removeCustomPropAssignment,types,CustomStyleProvider,CustomStyleInterface as $customStyleInterfaceDefault,CustomStyleInterfaceInterface,documentWait as $documentWaitDefault,nativeShadow,nativeCssVariables,toCssText,rulesForStyle,isKeyframesSelector,forEachRule,applyCss,createScopeStyle,applyStylePlaceHolder,applyStyle,isTargetedBuild,getCssBuildType,findMatchingParen,processVariableAndFallback,setElementClassRaw,getIsExtends,gatherStyleText,splitSelectorList,templateMap as $templateMapDefault,scopingAttribute,processUnscopedStyle,isUnscopedStyle,update as $indexDefault$5,newContext,invariant as $invariantDefault,PolymerRedux as $polymerReduxDefault,thunk as $indexDefault$6,defaultMemoize,createSelectorCreator,createSelector,createStructuredSelector,Controller as $controllerDefault,setChannel,setChannels,getChannel,setBpm,connectUsb,connectBluetooth,addUniverse,removeUniverse,resetUniverseAndFixtures,addScene,runScene,removeScene,setSceneName,addAnimationToScene,removeAnimationFromScene,addFixtureToScene,addFixturesToScene,removeFixtureFromScene,resetAllFixtures,addAnimation,runAnimation,removeAnimation,setAnimationName,addKeyframe,addFixture,setFixtureAddress,setFixtureProperties,setAllFixtureProperties,resetFixtureProperties,removeFixture,removeFixtureFromEverywhere,addMidi,removeMidi,enableMidi,learnMidi,addMidiMapping,setMidiMappingActive,addSceneToMidi,addScenesToMidi,removeSceneFromMidi,playTimeline,setTimelineProgress,resetTimeline,addSceneToTimeline,removeSceneFromTimeline,removeSceneFromTimelineAndResetFixtures,setLive,sendUniverseToUsb,sendUniverseToFivetwelve,setModvColor,connectModv,setDekkData,connectDekk,connectFivetwelve,TapButton,SET_CHANNEL,SET_CHANNELS,GET_CHANNEL,SET_BPM,SET_LIVE,CONNECT_USB,CONNECT_BLUETOOTH,ADD_UNIVERSE,REMOVE_UNIVERSE,DEMO_UNIVERSE,ADD_SCENE,RUN_SCENE,REMOVE_SCENE,SET_SCENE_NAME,ADD_ANIMATION_TO_SCENE,REMOVE_ANIMATION_FROM_SCENE,ADD_FIXTURE_TO_SCENE,REMOVE_FIXTURE_FROM_SCENE,ADD_ANIMATION,RUN_ANIMATION,REMOVE_ANIMATION,SET_ANIMATION_NAME,ADD_KEYFRAME,ADD_FIXTURE,SET_FIXTURE_ADDRESS,SET_FIXTURE_PROPERTIES,SET_ALL_FIXTURE_PROPERTIES,RESET_FIXTURE_PROPERTIES,REMOVE_FIXTURE,ENABLE_MIDI,ADD_MIDI,REMOVE_MIDI,ADD_MIDI_MAPPING,SET_MIDI_MAPPING_ACTIVE,LEARN_MIDI,ADD_SCENE_TO_MIDI,ADD_SCENES_TO_MIDI,REMOVE_SCENE_FROM_MIDI,ADD_SCENE_TO_TIMELINE,REMOVE_SCENE_FROM_TIMELINE,PLAY_TIMELINE,RESET_TIMELINE,SET_TIMELINE_PROGRESS,SEND_UNIVERSE_TO_USB,SET_MODV_COLOR,CONNECT_MODV,CONNECT_DEKK,SET_DEKK_DATA,CONNECT_FIVETWELVE,SEND_UNIVERSE_TO_FIVETWELVE,STORAGE_STATE,FIXTURE_TYPES,FIXTURE_PROPERTIES,MIDI_TYPE_BUTTON,MIDI_TYPE_KNOB,MIDI_TYPE_FADER,MIDI_TYPE_EMPTY,MIDI_TYPES,bpm,live,usbManager,modvManager,dekkManager,fivetwelveManager,connectionManager,universeManager,sceneManager,animationManager,fixtureManager,midiManager,timelineManager,store,reduxMixin,reduxMixin as $reduxStoreDefault,getScenes,getAnimations,getFixtures,getTimeline,getTimelineSceneIds,getAnimation,getAnimationsSorted,getScene,getSceneByName,getScenesSorted,getTimelineScenes,getFixture,AdjStarburst,CameoFlatPar1RGBW,CameoPixBar600PRO,CameoWookie200RGY,EuroliteB100,EuroliteTMH8,FunGenerationSeParQuadLedRgbUv,JsFestMovingHeadWash,StairvilleAF150,StairvilleBowlBeam604LEDCOBMovingHead,StairvilleOutdoorStageParTri,BasicColor,BasicFluter,AdjStarburst as $AdjStarburstDefault,BasicColor as $BasicColorDefault,BasicFluter as $BasicFluterDefault,CameoFlatPar1RGBW as $CameoFlatPar1RGBWDefault,CameoPixBar600PRO as $CameoPixBar600PRODefault,CameoWookie200RGY as $CameoWookie200RGYDefault,DmxDevice$2 as $DmxDeviceDefault$1,EuroliteB100 as $EuroliteB100Default,EuroliteTMH8 as $EuroliteTMH8Default,FunGenerationSeParQuadLedRgbUv as $FunGenerationSeParQuadLedRgbUvDefault,JsFestMovingHeadWash as $JsFestMovingHeadWashDefault,StairvilleAF150 as $StairvilleAF150Default,StairvilleBowlBeam604LEDCOBMovingHead as $StairvilleBowlBeam604LEDCOBMovingHeadDefault,StairvilleOutdoorStageParTri as $StairvilleOutdoorStageParTriDefault,batch,addToBatch,clearBatch,fixtureBatch,addToFixtureBatch,clearFixtureBatch,clearFixtureInBatch,modvData,setModvData,collator};